...
export function SystemOptimizer() {
        ...
        // Compute score & suggestions during render via useMemo — avoids
        // react-hooks/set-state-in-effect (no setState in effect needed).
        // The calculation is pure (depends only on devices + connections).
        const { optimizationScore, suggestions } = React.useMemo(() => {
                if (devices.length === 0) {
                        return { optimizationScore: 0, suggestions: [] };
                }
                ...
                return { optimizationScore: score, suggestions: newSuggestions };
        }, [devices, connections]);

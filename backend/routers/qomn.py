...
@router.post(
    ...
    responses={
        503: {"description": "QOMN kernel unavailable"},
        500: {"description": "Computation or validation error"},
...
async def run_golden_tests(request: Request):
    ...
    _test("NFPA72_heat_spacing_50m2", r3.spacing_m, expected_heat, 1e-6, "NFPA 72-2022 §17.6.3.1")
...
class AcousticsEvaluationRequest(BaseModel):
    ...
    speaker_spl_dba: float = Field(
        95.0, ge=60, le=120, description="Speaker rated SPL in dBA at 3m"
    )
    check_point_distance_m: float = Field(
        5.0, ge=0.5, le=50.0, description="Distance from speaker to check point"
    )

...
class ClusterCommunicator:
    ...
    def __init__(
        ...
        self.cluster_secret = os.getenv("FACP_CLUSTER_SECRET", "") or os.getenv(
            "FACP_INTER_NODE_SECRET", ""
        )
    ...
    def _handle_incoming_message(
        ...
        cluster_secret = (
            self.cluster_secret
            or os.getenv("FACP_CLUSTER_SECRET", "")
            or os.getenv("FACP_INTER_NODE_SECRET", "")
        )
        if cluster_secret:
            ...
            expected_sig = hmac.new(
                ...
                f"{sender_node_id}:{msg_type}:{ts}".encode(),
                hashlib.sha256,
            ).hexdigest()
            if not hmac.compare_digest(sig, expected_sig):
                print(
                    f"Rejected invalid HMAC cluster message signature from {sender_node_id}@{addr}"
                )
                return
    ...
    def send_message(self, node_id: str, message: dict[str, Any]) -> bool:
        ...
        if cluster_secret:
            ...
            message["hmac_sig"] = hmac.new(
                cluster_secret.encode("utf-8"), sig_base.encode("utf-8"), hashlib.sha256
            ).hexdigest()

...
# ============================================================================
# V10 ENHANCED - ADDITIONS FROM V11
# ============================================================================
# These components are extracted from V11 and added to V10 for enhanced functionality
# They do NOT modify V10's core logic

# Additional imports needed for resilience check
import random

# Monte Carlo constants (from V11)
...
def _run_resilience_check(
    ...
    return ResilienceResult(
        ...
        failure_detail    = (
            ...
            f"(min post-failure coverage {min_cov_seen*100:.1f}% < {floor*100:.0f}%)"
        ),
    )
...
def _sign_proof(result, poly, occupancy):
    """
    Placeholder for proof signing - requires RegulatoryProofEngine from gkil/proof_engine.
    This is optional and only runs when the engine is available.
    """
    # Placeholder - actual implementation requires RegulatoryProofEngine
...
def analyse_room_enhanced(
    ...
    if ceiling_type.lower() == 'beamed':
        ceiling_type_enum = V10CeilingType.BEAMED
    elif ceiling_type.lower() == 'sloped':
        ceiling_type_enum = V10CeilingType.SLOPED
    elif ceiling_type.lower() == 'corridor':
        ceiling_type_enum = V10CeilingType.CORRIDOR

...
export function MeezaPayment() {
    ...
    const fallbackToRedirect = useCallback(() => {
        ...
        setPhase({ kind: "redirect", checkout: phase.checkout, order: phase.order });
    ...
    return (
        <div className="w-full max-w-3xl mx-auto p-6 bg-white rounded-xl shadow-sm border border-slate-200">
            ...
            {(phase.kind === "creating_order" || phase.kind === "initiating_checkout") && (
                <LoadingState
                    label={
            ...
            {phase.kind === "success" && (
                <SuccessState order={phase.order} onDone={() => setPhase({ kind: "select" })} />
            )}
        ...
        <div className="space-y-4">
            <div className="p-3 bg-slate-50 border border-slate-200 rounded-md text-sm">
                ...
                <div className="flex items-center justify-between mt-1">
                    ...
                    <span className="text-slate-800 uppercase text-xs">{checkout.method}</span>
                </div>
            </div>
...
function PollingState({
    ...
    return (
        <div className="py-12 flex flex-col items-center gap-3">
            ...
            <p className="text-slate-700 font-medium">Waiting for payment confirmation...</p>
            <p className="text-sm text-slate-500">
    ...
    return (
        <div className="py-10 flex flex-col items-center gap-3">
            <div className="w-16 h-16 bg-rose-100 rounded-full flex items-center justify-center">
                <svg viewBox="0 0 24 24" fill="none" className="w-10 h-10 text-rose-600">
                    <path

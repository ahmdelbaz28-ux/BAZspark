...
export const AuditVisualizer: React.FC = () => {
	...
	useEffect(() => {
		...
		const hierarchyData: Record<string, any> = {
			...chain[0],
			name:
				((chain[0].data as Record<string, any>).event as string) || "GENESIS",
		...
		for (let i = 1; i < chain.length; i++) {
			const nodeData: Record<string, any> = {
				...chain[i],
				name: (chain[i].data as Record<string, any>).event as string,
				children: [] as Record<string, any>[],
			};
		...
		node
			...
			.attr(
				"fill",
				verifyStatus?.valid ? "hsl(var(--primary))" : "hsl(var(--destructive))",
			);
	...
	return (
		<Card className="h-full flex flex-col">
			<CardHeader>
				...
				<CardDescription>
					Cryptographic tamper-evident trail of all engineering decisions (NFPA
					72 §10.6)
				</CardDescription>
			</CardHeader>
			<CardContent className="flex-1 flex flex-col gap-4 min-h-[400px]">
				{verifyStatus && (
					<Alert
						variant={verifyStatus.valid ? "default" : "destructive"}
						className="shrink-0"
					>
						{verifyStatus.valid ? (
				...
				<div
					ref={wrapperRef}
					className="flex-1 bg-muted/20 rounded-md overflow-hidden border"
				>

...
export const OnboardingTour: React.FC = () => {
	...
	useEffect(() => {
		...
		// (setTargetElement) happens after a microtask — avoids
		// react-hooks/set-state-in-effect. We use Promise.resolve()
		// because getBoundingClientRect needs the layout to be committed
		// before it returns meaningful values.
		let cancelled = false;
		const readTarget = () => {
			...
			>
				<div className="p-4">
					<div className="flex items-start justify-between mb-2">
						<h3 className="text-primary font-semibold text-sm">{step.title}</h3>
						<button
							type="button"
							onClick={skipTour}

...
import { Lock, Settings, Server, Key } from 'lucide-react';
...
export const SettingsRegistry: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        const headers = { 'X-API-Key': process.env.VITE_FIREAI_API_KEY || 'test-key' };
  ...
  const handleToggle = async (key: string, checked: boolean) => {
    ...
    if (key === 'SMOKE_SIMULATION' || key === 'SELF_LEARNING') {
      alert(`Safety Override: ${key} is Disabled by V8 architecture and cannot be enabled via UI.`);
      return;
    }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">System Settings Registry</h2>
        <p className="text-muted-foreground">Three-Tier Security Architecture Configuration</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4">
          {Object.entries(runtime).map(([key, value]) => {
            const isDisabled = key === 'SMOKE_SIMULATION' || key === 'SELF_LEARNING';
            return (
                ...
                <div className="space-y-0.5">
                  ...
                  {isDisabled && (
                    <p className="text-xs text-destructive flex items-center gap-1 mt-1">
                      <Lock className="h-3 w-3" /> Disabled by V8 - Safety Constraint
                    </p>
                  )}
                </div>
                <Switch 
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Read-only environment and system configuration</CardDescription>
        </CardHeader>
      ...
      <Alert variant="default" className="bg-primary/5 border-primary/20">
        ...
        <AlertDescription>
          API keys, database URLs, and session secrets are managed via strict Separation of Privileges 
          and are never exposed to the UI or stored in the frontend runtime.
        </AlertDescription>
      </Alert>
    </div>
  );

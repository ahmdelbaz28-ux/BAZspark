	...
	async calculateQOMN(params: QOMNCalculationRequest): Promise<QOMNCalculationResult> {
		try {
			...
			return {
				...
				recommendations: [error instanceof Error ? error.message : "Network error during calculation"],
			};
		}
	}

	async calculateSmokeSpacing(length: number, width: number, height: number): Promise<QOMNCalculationResult> {
		return this.calculateQOMN({ room_length: length, room_width: width, ceiling_height: height });
	}

	async calculateBatteryBackup(loadAh: number, standbyHours: number, alarmMinutes: number): Promise<QOMNCalculationResult> {
		return this.calculateQOMN({ battery_load_ah: loadAh, standby_hours: standbyHours, alarm_minutes: alarmMinutes });
	}

	async calculateVoltageDrop(voltage: number, current: number, length: number, gauge: string): Promise<QOMNCalculationResult> {
		return this.calculateQOMN({ ps_voltage: voltage, ps_current: current, wire_length: length, wire_gauge: gauge });
	}
}

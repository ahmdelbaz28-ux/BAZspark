...
describe("BatteryCalculator", () => {
	describe("calculateBatteryRequirements", () => {
		...
		it("should recommend a 12V battery for small capacities (<20 Ah)", () => {
			const smallInput = {
				...validInput,
				devices: [
					{ type: "smoke", standbyCurrent: 0.01, alarmCurrent: 10, count: 1 },
				],
				standbyHours: 24,
				alarmMinutes: 5,
				safetyFactor: 1.2,
	...
	describe("getNFPA27_6_2Requirements", () => {
		it("should return a list of NFPA 72 §27.6.2 requirements", () => {
			...
			expect(reqs).toContain("Minimum 24 hours of standby operation");
			expect(reqs).toContain("Minimum 5 minutes of alarm operation");
		});

		it("should include the safety factor recommendation", () => {
			const reqs = getNFPA27_6_2Requirements();
			expect(reqs).toContainEqual(expect.stringContaining("20% safety factor"));
		});

...
export function useProjects(): {
	...
	const result = useQuery({
		...
		queryFn: () => api.getProjects().then((r) => unwrapPaginated<Project>(r)),
		staleTime: 30_000,
	});
...
export function useProject(id: string | null): {
	...
	const result = useQuery({
		...
		queryFn: () => api.getProject(id!).then((r) => unwrapData<Project>(r)),
		enabled: id !== null,
		staleTime: 30_000,
	});
	...
	const mutation = useMutation({
		mutationFn: (id: string) =>
			api.deleteProject(id).then((r) => {
				if (!r.success) throw new Error(r.error ?? "Failed to delete project");
			}),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: queryKeys.projects });
		},

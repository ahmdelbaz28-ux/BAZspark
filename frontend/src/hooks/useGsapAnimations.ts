...
export function useGsapLogoAnimation(
	...
	useGSAP(
		() => {
			...
			if (!DrawSVGPlugin) {
				console.warn(
					"[GSAP] DrawSVGPlugin not available - logo draw animation disabled",
				);
				return;
			}
	...
	useGSAP(
		() => {
			...
			targets.forEach((el) => {
				...
				const splitType = type.includes("chars")
					? split.chars
					: type.includes("words")
						? split.words // NOSONAR - typescript:S3358: nested ternary intentional for split type selection
						: split.lines;
...
export function useGsapMotionPath(
	...
	useGSAP(
		() => {
			...
			const targets =
				containerRef.current.querySelectorAll(".gsap-motion-path");
...
function formatCounterValue(
	...
	const formatted =
		finalValue % 1 === 0 ? Math.round(obj.val) : obj.val.toFixed(1);

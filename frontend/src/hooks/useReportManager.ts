...
function generateReportSections(
	// NOSONAR — S3776: report generation must format many report types
	type: ReportType,
	devices: DeviceData[],
	connections: ConnectionData[],
			...
			const checks = [
				{
					...
					detail:
						"Verify ampacity per continuous load (125%) + non-continuous (100%). Use cable_sizing report for automated NEC §310.16 check.",
				},
				{
					rule: "NEC 250 - Grounding",
					status: "MANUAL REVIEW REQUIRED",
					detail:
						"Verify grounding electrode system + bonding. Use grounding_analysis report for IEEE 80 resistance calculation.",
				},
				{
					rule: "NEC 300.3 - Conductors in Raceway",
					status: "MANUAL REVIEW REQUIRED",
					detail:
						"Verify all conductors of same circuit in same raceway. Manual field verification required.",
				},
				{
					rule: "NEC 310.15 - Ampacity",
					status: "MANUAL REVIEW REQUIRED",
					detail:
						"Verify ampacity after ambient temperature correction + derating. Use cable_sizing report for automated check.",
				},
				{
					rule: "NEC 700 - Emergency Systems",
					status: "MANUAL REVIEW REQUIRED",
					detail:
						"Verify emergency circuit identification + separation. Manual verification required per NEC 700.10.",
				},
			];
			...
			return [
				{
					...
					data: {
						...
						disclaimer:
							"Automated compliance verification is not performed in this template. All rules require manual review by a licensed engineer. For automated NEC ampacity checking, use the cable_sizing report. For NFPA 72 detector spacing verification, use the nfpa72_coverage report.",
					},
				},
				{
...
export function exportReport(
	// NOSONAR — S3776: multi-format export must handle JSON/PDF/CSV/print pathways

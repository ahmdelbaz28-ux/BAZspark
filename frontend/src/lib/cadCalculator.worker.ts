...
function newtonRaphson2Bus(
	...
	for (let iter = 0; iter < maxIter; iter++) {
		...
		const P2_calc = V2 * V2 * G22 - V1_pu * V2 * (G21 * cosT + B21 * sinT);
		const Q2_calc = -V2 * V2 * B22 - V1_pu * V2 * (G21 * sinT - B21 * cosT);
		...
		const correction = solve2x2([J11, J12, J21, J22], [-dP, -dQ]);
		if (!correction) {
		...
		if (V2 < 0.5 || V2 > 1.5 || !isFinite(V2)) {
			return {
				V2: Math.max(0.5, Math.min(1.5, V2 || 1.0)),
				theta2,
				iterations: iter + 1,
				converged: false,
			};

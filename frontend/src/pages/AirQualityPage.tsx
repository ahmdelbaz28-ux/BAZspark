...
import { AlertTriangle, Info, Loader2, Search, Wind } from "lucide-react";
...
export const AirQualityPage: React.FC = () => {
	...
	const fetchAQ = async () => {
		...
		try {
			...
			setError(
				err instanceof Error ? err.message : "Failed to fetch air quality",
			);
		} finally {
			setLoading(false);
		}
			...
			<div className="p-6 max-w-4xl mx-auto space-y-6">
					...
					<div className="space-y-4">
						...
						<div className={`rounded-xl p-6 border ${bgClass}`}>
							...
							<div className="flex items-end gap-4">
								<div>
									<div className={`text-5xl font-bold font-mono ${colorClass}`}>
										{data.aqi}
									</div>
									<div
										className={`text-sm font-medium mt-1 capitalize ${colorClass}`}
									>
										{data.aqi_level}
									</div>
								</div>
				...
				<div className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-4">
					...
					<p className="text-[11px] text-slate-500 mt-2">
						Air quality data sourced from WAQI & Open-Meteo Air Quality API.
						Moderate AQI (100) is used as conservative default when external
						endpoints are offline per NFPA 72 / NFPA 130 safety protocol.
					</p>
				</div>
			</div>
		</div>

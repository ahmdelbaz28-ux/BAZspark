...
export const AnalysisPage: React.FC = () => {
  ...
  const tabs: { key: typeof selectedAnalysis; label: string; icon: React.ElementType; desc: string }[] = [
    { key: "battery", label: "Battery Capacity", icon: Battery, desc: "NFPA 72 standby battery sizing" },
    { key: "voltage", label: "Voltage Drop", icon: Zap, desc: "NEC Chapter 9 Table 8 voltage drop" },
    { key: "room", label: "Room Analysis", icon: Building2, desc: "Full room coverage & detection analysis" },
  ];
      ...
      <div className="p-6 max-w-4xl mx-auto space-y-6">
        ...
        {batteryMutation.isSuccess && batteryMutation.data?.data && (
          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: "Required Capacity", value: `${batteryMutation.data.data.required_capacity_ah.toFixed(1)} Ah` },
                { label: "Selected Battery", value: `${batteryMutation.data.data.selected_battery_ah} Ah` },
                { label: "Backup Time", value: `${batteryMutation.data.data.backup_minutes} min` },
                { label: "Margin", value: `${batteryMutation.data.data.margin_percent.toFixed(1)}%` },
              ].map((m) => (
                <div key={m.label} className="bg-slate-800 rounded-lg p-3 border border-slate-700/50 text-center">
                  <div className="text-[10px] text-slate-500 mb-1">{m.label}</div>
                  <div className="text-sm font-bold text-slate-100 font-mono">{m.value}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {voltageMutation.isSuccess && voltageMutation.data?.data && (
          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: "Drop %", value: `${voltageMutation.data.data.drop_percent.toFixed(2)}%` },
                { label: "Drop (V)", value: `${voltageMutation.data.data.drop_volts.toFixed(3)} V` },
                { label: "Status", value: voltageMutation.data.data.passes ? "PASS" : "FAIL" },
                { label: "Recommended Wire", value: voltageMutation.data.data.recommended_wire },
              ].map((m) => (
                <div key={m.label} className="bg-slate-800 rounded-lg p-3 border border-slate-700/50 text-center">
                  <div className="text-[10px] text-slate-500 mb-1">{m.label}</div>
        ...
        {roomMutation.isSuccess && roomMutation.data?.data && (
          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-5">
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                { label: "Detectors", value: String(roomMutation.data.data.detector_count) },
                { label: "Coverage", value: `${roomMutation.data.data.coverage_percent}%` },
                { label: "Notifications", value: String(roomMutation.data.data.notifications) },
              ].map((m) => (
                <div key={m.label} className="bg-slate-800 rounded-lg p-3 border border-slate-700/50 text-center">
                  <div className="text-[10px] text-slate-500 mb-1">{m.label}</div>
                  <div className="text-sm font-bold text-slate-100 font-mono">{m.value}</div>

...
import { Check, Key, Loader2, Pencil, Plus, Trash2, X } from "lucide-react";
	...
	const handleDelete = async (keyHash: string) => {
		...
		const deletePromise = new Promise<void>((resolve, reject) => {
			...
			toast("Delete this API key? This cannot be undone.", {
				...
				cancel: {
					label: "Cancel",
					onClick: () => rejectFn?.(new Error("Cancelled")),
				},
									...
									{editingKeyHash === key.key_hash ? (
										...
										<div className="space-y-1">
											<div className="flex items-center gap-2">
												...
												<Badge className={roleColor(key.role)}>
													{key.role}
												</Badge>
											</div>
											<p className="text-xs text-muted-foreground">
												{key.description || "No description"} · Created{" "}
										...
										{editingKeyHash === key.key_hash ? (
											<>
												<Button
													variant="ghost"
													size="sm"
													onClick={() => handleSaveRole(key.key_hash)}
													disabled={savingRole}
												...
												<Button
													variant="ghost"
													size="sm"
													onClick={() => setEditingKeyHash(null)}
													disabled={savingRole}
												>
													<X className="h-4 w-4" />
												</Button>

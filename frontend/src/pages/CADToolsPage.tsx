...
export const CADToolsPage: React.FC = () => {
	...
	const [activeTab, setActiveTab] = useState<"connect" | "draw" | "file">(
		"connect",
	);
	...
	const statusMutation = useMutation({
		mutationFn: async () => {
			return cadApi.getStatus() as Promise<{
				success: boolean;
				provider: string;
				status: CADStatus;
			}>;
	...
	const drawLineMutation = useMutation({
		mutationFn: async () => {
			...
			return cadApi.drawLine({
				start_x: sx,
				start_y: sy,
				end_x: ex,
				end_y: ey,
				layer: drawLineLayer,
			}) as Promise<DrawResult>;
		},
	});
	...
	const drawCircleMutation = useMutation({
		mutationFn: async () => {
			...
			return cadApi.drawCircle({
				center_x: cx,
				center_y: cy,
				radius: Number.parseFloat(circleRadius) || 1,
				layer: circleLayer,
			}) as Promise<DrawResult>;
		},
	});
	...
	const drawTextMutation = useMutation({
		mutationFn: async () => {
			...
			return cadApi.drawText({
				x: ix,
				y: iy,
				text: textContent,
				height: Number.parseFloat(textHeight) || 0.2,
				layer: textLayer,
			}) as Promise<DrawResult>;
		},
	});
	...
	const writeMutation = useMutation({
		mutationFn: async () => {
			return cadApi.write({
				...
				format: provider,
				data: { elements: readResult?.elements || [] },
			...
			<div className="p-6 max-w-5xl mx-auto space-y-6">
				...
				<div>
					...
					<p className="text-slate-400 text-sm mt-1">
						Connect to AutoCAD or Revit, manage drawings, and execute CAD
						operations
					</p>
				</div>
				...
				<div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
					<div className="flex flex-col sm:flex-row sm:items-center gap-4">
						...
						<div className="flex items-center gap-3">
							...
							>
								<Loader2
									className={`h-3.5 w-3.5 ${statusMutation.isPending ? "animate-spin" : ""}`}
								/>
								Status
							</button>
						</div>
					...
					<div className="bg-slate-800/50 border border-slate-700 rounded-xl p-5">
						...
						<div className="bg-slate-800/30 border border-slate-700/30 rounded-lg p-3">
							<p className="text-xs text-slate-500">
								<strong className="text-slate-400">Note:</strong> Connection
								uses simulation mode by default. Set{" "}
								<code className="text-cyan-300">visible=true</code> and{" "}
								<code className="text-cyan-300">method=simulation</code> to see
								the CAD application window (simulated). For real CAD
								connections, configure the provider path in CAD Settings.
							</p>
						</div>
					</div>
				)}
				...
				{activeTab === "draw" && (
					<div className="space-y-4">
						...
						<div className="bg-slate-800/50 border border-slate-700 rounded-xl p-5">
							...
							<div className="grid grid-cols-3 gap-3 mb-3">
								<CoordInput
									label="Start (x,y,z)"
									value={drawLineStart}
									onChange={setDrawLineStart}
								/>
								<CoordInput
									label="End (x,y,z)"
									value={drawLineEnd}
									onChange={setDrawLineEnd}
								/>
								<div>
							...
							{drawLineMutation.isSuccess && (
								<p className="text-xs text-emerald-400 mt-2 flex items-center gap-1">
									<CheckCircle2 className="h-3 w-3" />
									Handle:{" "}
									{(drawLineMutation.data as DrawResult)?.handle || "N/A"}
								</p>
							)}
						</div>
						...
						<div className="bg-slate-800/50 border border-slate-700 rounded-xl p-5">
							...
							{drawPolylineMutation.isSuccess && (
								<p className="text-xs text-emerald-400 mt-2 flex items-center gap-1">
									<CheckCircle2 className="h-3 w-3" />
									Handle:{" "}
									{(drawPolylineMutation.data as DrawResult)?.handle || "N/A"}
								</p>
							)}
						</div>
						...
						<div className="bg-slate-800/50 border border-slate-700 rounded-xl p-5">
							...
							{drawCircleMutation.isSuccess && (
								<p className="text-xs text-emerald-400 mt-2 flex items-center gap-1">
									<CheckCircle2 className="h-3 w-3" />
									Handle:{" "}
									{(drawCircleMutation.data as DrawResult)?.handle || "N/A"}
								</p>
							)}
						</div>
						...
						<div className="bg-slate-800/50 border border-slate-700 rounded-xl p-5">
							...
							<div className="mb-3">
								<CoordInput
									label="Insertion Point (x,y,z)"
									value={textInsertion}
									onChange={setTextInsertion}
								/>
							</div>
							<ActionButton
								mutation={{
							...
							{drawTextMutation.isSuccess && (
								<p className="text-xs text-emerald-400 mt-2 flex items-center gap-1">
									<CheckCircle2 className="h-3 w-3" />
									Handle:{" "}
									{(drawTextMutation.data as DrawResult)?.handle || "N/A"}
								</p>
							)}
						</div>
					</div>
											...
											<table className="w-full text-xs">
												...
												<tbody>
													{readResult.elements.slice(0, 20).map((el, i) => (
														<tr
															key={i}
															className="border-b border-slate-700/30 text-slate-400"
														>
							...
							<div className="flex items-start gap-2">
								...
								<div>
									...
									<p className="text-xs text-amber-300/70">
										Connect to a provider (AutoCAD or Revit) before reading or
										writing drawing files. Only .dwg, .dxf, .rvt, .rfa, and .ifc
										files are allowed. File paths are validated against path
										traversal attacks.
									</p>
								</div>
							</div>
					...
					<div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
						<div className="flex items-start gap-2">
							...
							<p className="text-xs text-amber-400">
								Connect to {provider === "autocad" ? "AutoCAD" : "Revit"} first
								to enable drawing tools. Use the Connection panel above to

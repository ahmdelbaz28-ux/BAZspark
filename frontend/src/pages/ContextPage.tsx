...
import { Wind, Search, MapPin, Loader2, Globe, Thermometer, Droplets, Landmark } from "lucide-react";
      ...
      <div className="p-6 max-w-5xl mx-auto space-y-6">
              ...
              <div className="mt-4 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-slate-700/30 rounded-lg p-3 text-center">
                    ...
                    <div className="text-lg font-bold text-slate-100 font-mono">{weather.temperature_c}°C</div>
                    <div className="text-[10px] text-slate-500">Temperature</div>
                  </div>
                  <div className="bg-slate-700/30 rounded-lg p-3 text-center">
                    <Wind className="h-4 w-4 text-cyan-400 mx-auto mb-1" />
                    <div className="text-lg font-bold text-slate-100 font-mono">{weather.wind_speed_m_s} m/s</div>
                    <div className="text-[10px] text-slate-500">Wind Speed</div>
                  </div>
                  <div className="bg-slate-700/30 rounded-lg p-3 text-center">
                    <Droplets className="h-4 w-4 text-cyan-400 mx-auto mb-1" />
                    <div className="text-lg font-bold text-slate-100 font-mono">{weather.relative_humidity_pct}%</div>
                    <div className="text-[10px] text-slate-500">Humidity</div>
                  </div>
                  <div className="bg-slate-700/30 rounded-lg p-3 text-center">
                    <Wind className="h-4 w-4 text-cyan-400 mx-auto mb-1" />
                    <div className="text-lg font-bold text-slate-100 font-mono">{weather.air_density_kg_m3} kg/m³</div>
                    <div className="text-[10px] text-slate-500">Air Density</div>
                  </div>
                </div>
          ...
          <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-5">
            ...
            {geocode && (
              <div className="space-y-2">
                <div className="bg-slate-700/30 rounded-lg p-3">
                  ...
                  <p className="text-xs text-slate-200">{geocode.display_name}</p>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-slate-700/30 rounded-lg p-2.5 text-center">
                    <div className="text-[10px] text-slate-500">Lat</div>
                    <div className="text-xs font-mono text-slate-200">{geocode.latitude.toFixed(4)}</div>
                  </div>
                  <div className="bg-slate-700/30 rounded-lg p-2.5 text-center">
                    <div className="text-[10px] text-slate-500">Lon</div>
                    <div className="text-xs font-mono text-slate-200">{geocode.longitude.toFixed(4)}</div>
                  </div>
                </div>
                {region && (
                  <>
                    <div className="border-t border-slate-700 pt-2 mt-2">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className="bg-slate-700/30 rounded-lg p-2">
                          ...
                          <div className="text-xs text-slate-200">{region.country_name}</div>
                        </div>
                        <div className="bg-slate-700/30 rounded-lg p-2">
                          <div className="text-[10px] text-slate-500">Framework</div>
                          <div className="text-xs text-slate-200">{region.regulatory_framework}</div>
                        </div>
                        <div className="bg-slate-700/30 rounded-lg p-2">
                          <div className="text-[10px] text-slate-500">Electrical Code</div>
                          <div className="text-xs text-slate-200">{region.electrical_code}</div>
                        </div>
                        <div className="bg-slate-700/30 rounded-lg p-2">
                          <div className="text-[10px] text-slate-500">Country Code</div>
                          <div className="text-xs text-slate-200 font-mono">{region.country_code}</div>
                        </div>
                      </div>
                    </div>
                  </>
            ...
            {!geocode && !geoLoading && (
              <div className="flex flex-col items-center justify-center py-8 text-slate-500">
                ...
                <p className="text-xs">Search for an address to see location and regulatory data</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-4">
          <p className="text-xs text-slate-500">
            Weather data sourced from Open-Meteo (free, no auth). Geocoding via Nominatim/OpenStreetMap.
            Regulatory region data provides applicable fire and electrical codes for engineering calculations.
          </p>
        </div>
      </div>
    </div>

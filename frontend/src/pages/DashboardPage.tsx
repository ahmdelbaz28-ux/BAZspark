...
export function DashboardPage() {
	...
	const { data: devices, loading: devicesLoading } = useDevices(null); // Pass null as projectId
	...
	return (
		<div className="flex-1 overflow-auto" aria-label={t("dashboard.title")}>
			...
			<div className="p-6 max-w-7xl mx-auto space-y-8">
				...
				{!connected && !healthLoading && !troubleAcked && (
					...
					>
						...
						<span className="dashboard-trouble-banner-detail">
							Backend connection lost — system operating in offline mode. Live
							data unavailable.
						</span>
						<button
							type="button"
							...
							<div className="flex items-center gap-3">
								...
								<div>
									...
									<div className="text-sm text-muted-foreground">
										{okDevices} {t("dashboard.devices").toLowerCase()}{" "}
										{t("dashboard.acrossAllProjects").toLowerCase()}
									</div>
								</div>
							</div>
						) : (
				 ...
				 * ══════════════════════════════════════════════════════════════ */}
				<Card
					className="border-border bg-card dashboard-stagger-in"

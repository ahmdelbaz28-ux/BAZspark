...
export function DatabaseAdminPage() {
	// NOSONAR — S3776: multi-tab database admin must handle diverse DB operations
	const { toast } = useToast();
	const [loading, setLoading] = useState(false);
	const [activeTab, setActiveTab] = useState("health");
	const [health, setHealth] = useState<DbHealth | null>(null);
	const [mainHealth, setMainHealth] = useState<Record<string, unknown> | null>(
		null,
	);
	...
	const handleCheckHealth = async () => {
		...
		try {
			...
			toast({
				...
				description: multiRes
					? "Multi-db status retrieved"
					: "Multi-db endpoint unavailable",
			});
		} catch (err) {
			toast({
			...
			<div className="p-6 max-w-5xl mx-auto space-y-6">
				...
				<div>
					...
					<p className="text-sm text-muted-foreground mt-1">
						Monitor and manage PostgreSQL, Redis, Neo4j, and Qdrant database
						connections
					</p>
				</div>
						...
						<CardContent>
							{health === null ? (
								...
								<>
									<Badge
										variant={
											health["postgresql"] !== false ? "default" : "destructive"
										}
										className="text-xs"
									>
										{health["postgresql"] !== false
					...
					<Card className="border-border bg-card stagger-card">
						...
						<CardContent>
							{health === null ? (
								...
								<Badge
									variant={
										health["redis"] !== false ? "default" : "destructive"
									}
									className="text-xs"
								>
					...
					<Card className="border-border bg-card stagger-card">
						...
						<CardContent>
							{health === null ? (
								...
								<Badge
									variant={
										health["neo4j"] !== false ? "default" : "destructive"
									}
									className="text-xs"
								>
					...
					<Card className="border-border bg-card stagger-card">
						...
						<CardContent>
							{health === null ? (
								...
								<Badge
									variant={
										health["qdrant"] !== false ? "default" : "destructive"
									}
									className="text-xs"
								>
					...
					<TabsContent value="neo4j">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Execute predefined, read-only Cypher queries against the Neo4j
									topology graph
								</CardDescription>
							</CardHeader>
							<CardContent>
							...
							<CardContent className="py-12">
								<div className="flex flex-col items-center text-center">
									...
									<p className="text-sm text-muted-foreground/60 mt-1">
										Click "Refresh All Health" to check connection status for
										all databases
									</p>
								</div>
							</CardContent>

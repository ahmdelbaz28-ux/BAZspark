...
function getCableComplianceLevel(
...
export function EngineeringPage() {
	...
	const calculateVoltageDropViaApi = async () => {
		...
		try {
			...
			const msg =
				err instanceof Error ? err.message : "QOMN API calculation failed";
			setApiError(msg);
			setApiResult(null);
		} finally {
			setApiLoading(false);
	...
	const calculateCableSizing = useCallback(() => {
		...
		if (cableSizingInputs.installationMethod === "conduit")
			deratingFactor *= 0.8;
		else if (cableSizingInputs.installationMethod === "tray")
			deratingFactor *= 0.85;
		else if (cableSizingInputs.installationMethod === "buried")
			deratingFactor *= 0.75;
	...
	const calculateBatteryRequirements = useCallback(() => {
		const standbyDevices = Number.parseInt(
			batteryCalcInputs.standbyDevices,
			10,
		);
	...
	useEffect(() => {
		if (
			prevComplianceRef.current !== null &&
	...
	const hasVoltageInput = !!(
		voltageDropInputs.current || voltageDropInputs.length
	);
	const hasCableInput = !!(
		cableSizingInputs.loadCurrent || cableSizingInputs.length
	);
	const hasBatteryInput = !!(
		batteryCalcInputs.standbyDevices || batteryCalcInputs.alarmDevices
	);
					...
					{tabs.map((tab) => (
						<button
							...
							className={`etap-tab transition-colors duration-200 ${
								activeTab === tab
									? "text-[var(--etap-accent)] border-b-[var(--etap-accent)]"
									: ""
							}`}
							role="tab"
									...
									<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-4">
										...
										<div className="etap-field">
											...
											<Select
												...
												onValueChange={(v) =>
													setVoltageDropInputs((p) => ({ ...p, material: v }))
												}
											>
												<SelectTrigger
													aria-label={t("engineering.material")}
													className="etap-input"
												>
													<SelectValue />
												</SelectTrigger>
										...
										<div className="etap-panel-body space-y-3">
											...
											<div className="space-y-1.5">
												...
												{"computation_hash" in vDropResult &&
													vDropResult.computation_hash && (
														<div className="flex justify-between items-baseline">
															<span className="etap-label">
																{t("engineering.hash")}
															</span>
															<span
																className="font-mono-num text-[10px] text-[var(--etap-text-muted)] truncate max-w-[140px]"
																title={vDropResult.computation_hash as string}
															>
																{(vDropResult.computation_hash as string).slice(
																	0,
																	12,
																)}
																…
										...
										>
											...
											<div className="flex items-center gap-3 mb-2">
												{complianceLevel === "excessive" ? (
													<AlertTriangle
														aria-hidden="true"
														className="h-6 w-6 text-[var(--etap-danger)]"
													/>
												) : complianceLevel === "acceptable" ? (
													<ShieldCheck
														aria-hidden="true"
														className="h-6 w-6 text-[var(--etap-warning)]"
													/>
												) : (
													<ShieldCheck
														aria-hidden="true"
														className="h-6 w-6 text-[var(--etap-success)]"
													/>
												)}
												<span className="etap-label">
											...
											<div
												className={`font-mono-num font-bold text-3xl leading-tight tracking-tight mb-1 ${
													complianceLevel === "suitable"
														? "text-[var(--etap-success)]"
														: complianceLevel === "acceptable"
															? "text-[var(--etap-warning)]"
						...
						>
								...
								<div className="etap-panel-body">
									<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-4">
										...
										<div className="etap-field">
											...
											>
												<SelectTrigger
													aria-label={t("engineering.installationMethod")}
													className="etap-input"
												>
													<SelectValue />
												</SelectTrigger>
												<SelectContent>
													...
													<SelectItem value="tray">
														{t("engineering.cableTray")}
													</SelectItem>
													<SelectItem value="buried">
														{t("engineering.directBurial")}
													</SelectItem>
												</SelectContent>
											</Select>
										</div>
							...
							{hasCableInput && (
								<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
									...
									<div
										className={`etap-panel overflow-hidden border-l-4 ${
									...
									>
										<div className="p-5 h-full flex flex-col justify-center">
											...
											<div
												className={`font-mono-num font-bold text-2xl mb-1 ${
													cableComplianceLevel === "suitable"
														? "text-[var(--etap-success)]"
														: cableComplianceLevel === "acceptable"
															? "text-[var(--etap-warning)]"
									...
									<div className="etap-panel border-l-4 border-l-[var(--etap-accent)] bg-[var(--etap-accent-muted)]">
										<div className="p-5 h-full flex flex-col justify-center">
											<div className="flex items-center gap-3 mb-2">
												<Battery
													aria-hidden="true"
													className="h-6 w-6 text-[var(--etap-accent)]"
												/>
												<span className="etap-label">
													{t("engineering.recommendation")}
												</span>
											</div>
											...
											<div className="text-sm text-[var(--etap-text-secondary)]">
												NFPA 72 §10.6.7.2.1 —{" "}
												{t("engineering.standbyAlarmDesc", {
													hours: batteryCalcInputs.standbyHours,
													minutes: batteryCalcInputs.alarmMinutes,
												})}
											</div>
										</div>
									</div>
									...
									<div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
										...
										<div className="etap-field">
											...
											<Input
												...
												onChange={(e) =>
													setRoomInputs((p) => ({ ...p, zone: e.target.value }))
												}
												className="etap-input"
										...
										{integrationSubsystems.map(({ key, label }) => (
											...
											>
												{integrationToggles[key] ? (
													<CheckCircle2
														aria-hidden="true"
														className="h-4 w-4 text-[var(--etap-success)]"
													/>
												) : (
													<AlertTriangle
														aria-hidden="true"
														className="h-4 w-4 text-[var(--etap-danger)]"
													/>
												)}
												<span
													className={`font-mono-num text-sm ${
														integrationToggles[key]
															? "text-[var(--etap-text-secondary)]"
															: "text-[var(--etap-danger)]"
													}`}
												>
													{label}
									...
									<div className="mt-5">
										<Button
											onClick={() =>
												apiCall("/integration", {
													method: "POST",
													body: JSON.stringify(integrationToggles),
												})
											}
											className="etap-btn etap-btn-primary"
										>

...
import { Cloud, MapPin, AlertTriangle, Loader2, Search, Globe, Layers, Database } from "lucide-react";
...
export function EnvironmentPage() {
        ...
        const [fullContext, setFullContext] = useState<Record<string, unknown> | null>(null);
        ...
        const handleRegion = async () => {
                setLoading(true);
                try {
                        const res = await environmentApi.getRegion(Number.parseFloat(lat), Number.parseFloat(lon));
                        setRegion(res as Record<string, unknown>);
                } catch (err) {
                        toast({
                                title: "Region Lookup Failed",
                                description: err instanceof Error ? err.message : "Failed",
                                variant: "destructive",
                        });
                } finally {
        ...
        const handleContext = async () => {
                setLoading(true);
                try {
                        const res = await environmentApi.getContext(Number.parseFloat(lat), Number.parseFloat(lon));
                        setContext(res as Record<string, unknown>);
                } catch (err) {
                        toast({
        ...
        const handleFullContext = async () => {
                setLoading(true);
                try {
                        const res = await environmentApi.getFullContext(Number.parseFloat(lat), Number.parseFloat(lon));
                        setFullContext(res as Record<string, unknown>);
                } catch (err) {
                        toast({
                                title: "Full Context Lookup Failed",
                                description: err instanceof Error ? err.message : "Failed",
                                variant: "destructive",
                        });
                } finally {
                        ...
                        <div className="p-6 max-w-5xl mx-auto space-y-6">
                                <div>
                                        ...
                                        <p className="text-sm text-muted-foreground mt-1">
                                                Weather · Elevation · Air Quality · Severe Weather · Hazmat · Region · Context — for site-specific fire protection design
                                        </p>
                                </div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
                                        ...
                                        <Button onClick={handleRegion} disabled={loading} variant="outline">
                                                {loading ? <Loader2 aria-hidden="true" className="h-4 w-4 animate-spin" /> : <Globe aria-hidden="true" className="h-4 w-4" />}
                                                Region
                                        </Button>
                                        <Button onClick={handleContext} disabled={loading} variant="outline">
                                                {loading ? <Loader2 aria-hidden="true" className="h-4 w-4 animate-spin" /> : <Layers aria-hidden="true" className="h-4 w-4" />}
                                                Context
                                        </Button>
                                        <Button onClick={handleFullContext} disabled={loading} variant="outline">
                                                {loading ? <Loader2 aria-hidden="true" className="h-4 w-4 animate-spin" /> : <Database aria-hidden="true" className="h-4 w-4" />}
                                                Full Context
                                        </Button>
                                </div>
                                        ...
                                        {region && (
                                                <Card>
                                                        <CardHeader>
                                                                ...
                                                                <CardDescription>Regulatory region info (country, code, jurisdiction)</CardDescription>
                                                        </CardHeader>
                                                        <CardContent>{renderJson(region)}</CardContent>
                                                </Card>
                                        )}
                                        {context && (
                                                <Card>
                                                        <CardHeader>
                                                                ...
                                                                <CardDescription>Environmental context summary</CardDescription>
                                                        </CardHeader>
                                                        <CardContent>{renderJson(context)}</CardContent>
                                                </Card>
                                        )}
                                        {fullContext && (
                                                <Card>
                                                        <CardHeader>
                                                                ...
                                                                <CardDescription>Complete environmental context aggregation</CardDescription>
                                                        </CardHeader>
                                                        <CardContent>{renderJson(fullContext)}</CardContent>
                                                </Card>
                                        )}
                                </div>

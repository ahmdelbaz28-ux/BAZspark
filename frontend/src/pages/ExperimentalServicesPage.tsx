...
export function ExperimentalServicesPage() {
	...
	const [speckleResult, setSpeckleResult] = useState<ProcessResult | null>(
		null,
	);
	...
	useEffect(() => {
		...
		// (react-hooks/set-state-in-effect). `fetchFeatures` is still
		// defined above for use by event handlers (refresh).
		let cancelled = false;
		(async () => {
			try {
				const json = await apiCall<{ data?: { features?: FeatureStatus[] } }>(
				...
				if (!cancelled) setFeatures(json?.data?.features ?? []);
			} catch (err) {
				if (!cancelled) {
					toast({
						title: "Failed to load experimental features",
		...
		<div className="space-y-6 p-6">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-sm text-muted-foreground mt-1">
						Previously-orphan backend services now exposed for admin use. Marked
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Upload a scanned PDF or image. Returns extracted text and room names
						(eng+ara).
					</CardDescription>
				</CardHeader>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Upload a floor plan. Returns BIM rooms with areas, types, and
						confidence scores.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-3">
			...
			<Card>
				...
				<CardContent className="space-y-3">
					<div className="grid grid-cols-1 md:grid-cols-3 gap-3">
						<div>
							<Label htmlFor="speckle-stream" className="text-xs">
								Stream ID
							</Label>
							<Input
								id="speckle-stream"
						...
						<div>
							<Label htmlFor="speckle-server" className="text-xs">
								Server URL
							</Label>
							<Input
								id="speckle-server"
						...
						<div>
							<Label htmlFor="speckle-token" className="text-xs">
								API Token
							</Label>
							<Input
								id="speckle-token"

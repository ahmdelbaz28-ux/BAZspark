...
export function ExportsPage() {
	...
	const handleExportDxf = () => {
		downloadFile(
			...
			`${projectId}_export.dxf`,
		);
	};
	const handleExportRevit = () => {
		downloadFile(
			`/api/v1/projects/${projectId}/export/revit`,
			`${projectId}_revit.json`,
		);
	};
	const handleExportIfc = () => {
		downloadFile(
			`/api/v1/projects/${projectId}/export/ifc`,
		...
		<div className="flex-1 overflow-auto p-6 max-w-3xl mx-auto space-y-6">
			...
			<Card className="border-border bg-card stagger-card">
				<CardHeader>
					...
					<CardDescription>
						Each format produces a real file with project data
					</CardDescription>
				</CardHeader>

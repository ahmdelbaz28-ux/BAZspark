...
export function FACPPage() {
        ...
        const [extendedResult, setExtendedResult] = useState<Record<string, unknown> | null>(null);
                                        ...
                                        <CardContent>
                                                <div className="space-y-4">
                                                        ...
                                                        <div className="flex flex-wrap gap-3">
                                                                <Button
                                                                        onClick={async () => {
                                                                                if (!panelId) { toast({ title: "Panel ID required", variant: "destructive" }); return; }
                                                                                setLoading(true); setExtendedResult(null);
                                                                                try {
                                                                                        const res = await facpExtendedApi.verify({ panel_id: panelId });
                                                                                        setExtendedResult(res as Record<string, unknown>);
                                                                                        toast({ title: "Compliance verified" });
                                                                                } catch (err) {
                                                                                        toast({ title: "Verify failed", description: err instanceof Error ? err.message : "Failed", variant: "destructive" });
                                                                ...
                                                                <Button
                                                                        onClick={async () => {
                                                                                ...
                                                                                setLoading(true); setExtendedResult(null);
                                                                                try {
                                                                                        const res = await facpExtendedApi.generateSchedule({ panel_id: panelId });
                                                                                        setExtendedResult(res as Record<string, unknown>);
                                                                ...
                                                                <Button
                                                                        onClick={async () => {
                                                                                ...
                                                                                setLoading(true); setExtendedResult(null);
                                                                                try {
                                                                                        const res = await facpExtendedApi.generateSpec({ panel_id: panelId });
                                                                                        setExtendedResult(res as Record<string, unknown>);
                                                                                        toast({ title: "Spec generated" });
                                                                                } catch (err) {
                                                                                        toast({ title: "Spec failed", description: err instanceof Error ? err.message : "Failed", variant: "destructive" });
                                                                                } finally { setLoading(false); }
                                                                        }}
                                                                        disabled={loading || !panelId}
                                                                        variant="outline"

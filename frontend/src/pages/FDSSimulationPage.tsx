...
export function FDSSimulationPage() {
	...
	const [smokeState, setSmokeState] = useState<Record<string, unknown> | null>(
		null,
	);
	...
	const [simUpdateResult, setSimUpdateResult] = useState<Record<
		string,
		unknown
	> | null>(null);
	...
	const handleSubmitJob = async () => {
		if (!fdsInput || fdsInput.length < 10) {
			toast({
				title: "FDS input must be at least 10 characters",
				variant: "destructive",
			});
			return;
		}
			...
			toast({
				...
				description:
					err instanceof Error
						? err.message
						: "FDS service may not be configured",
				variant: "destructive",
			});
		} finally {
			setLoading(false);
	...
	const handleCreateSmokeState = async () => {
		...
		try {
			...
			toast({
				title: "Invalid smoke density points JSON",
				variant: "destructive",
			});
			return;
		}
			...
			<div className="p-6 max-w-5xl mx-auto space-y-6">
				...
				<div>
					...
					<p className="text-sm text-muted-foreground mt-1">
						Submit, monitor, and review Fire Dynamics Simulator (FDS) smoke
						simulation jobs
					</p>
				</div>
						...
						<CardContent>
							...
							{selectedJob.error && (
								<div className="p-3 rounded-lg bg-danger/10 border border-danger/30 mb-3">
									...
									<p className="text-xs text-danger/80 mt-1 font-mono">
										{selectedJob.error}
									</p>
								</div>
							)}
				...
				<Card>
					...
					<CardContent>
						...
						<div className="mt-4 space-y-1.5">
							<Label className="text-xs text-muted-foreground">
								Smoke Density Points{" "}
								<span className="font-mono">(JSON array)</span>
							</Label>
							<Textarea
							...
							<div className="flex gap-2 mt-2">
								...
								<Button
									onClick={async () => {
										...
										setLoading(true);
										try {
											const res = await v2ExtendedApi.updateSmokeSimulation({
												state: parsedState,
											});
				...
				{jobs.length > 0 && (
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Recently submitted FDS simulation jobs
							</CardDescription>
						</CardHeader>
						<CardContent>

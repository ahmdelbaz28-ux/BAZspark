...
export function GenerativeDesignPage() {
	...
	const handleGenerate = async () => {
		if (roomWidth <= 0 || roomLength <= 0 || roomHeight <= 0) {
			toast({
				title: "Dimensions must be greater than 0",
				variant: "destructive",
			});
			return;
		}
		if (roomWidth > 1000 || roomLength > 1000) {
			toast({
				title: "Dimensions cannot exceed 1000m",
				variant: "destructive",
			});
			return;
		}
			...
			toast({
				...
				description:
					err instanceof Error
						? err.message
						: "Generative layout engine may not be available",
				variant: "destructive",
			});
		} finally {
			setLoading(false);
			...
			<div className="p-6 max-w-5xl mx-auto space-y-6">
				...
				<div>
					...
					<p className="text-sm text-muted-foreground mt-1">
						Generate optimized fire alarm layout variants using AI-powered
								...
								<CardContent className="py-16">
									<div className="flex flex-col items-center text-center">
										...
										<p className="text-sm text-muted-foreground/60 mt-1">
											The engine will produce 3 optimized layouts:
											cost-minimized, standard, and safety-maximized
										</p>
									</div>
								</CardContent>
							</Card>
									...
									<CardHeader className="pb-3">
										<CardTitle className="flex items-center gap-2 text-lg">
											...
											<span
												className={
													VARIANT_COLORS[
														result.variants[result.recommended_index]
															?.variant_name || ""
													] || "text-foreground"
												}
											>
									...
									>
										<CardHeader className="pb-3">
											<div className="flex items-center justify-between">
												...
												<Badge
													variant={
														variant.code_compliant ? "default" : "destructive"
													}
													className="text-xs"
												>
													{variant.code_compliant
														? "Compliant"
														: "Non-Compliant"}
												</Badge>
											</div>

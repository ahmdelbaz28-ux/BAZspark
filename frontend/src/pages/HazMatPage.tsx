      ...
      <div className="p-6 max-w-4xl mx-auto space-y-6">
        ...
        <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-5">
          ...
          {!loadingKnown && knownMaterials.length > 0 && (
            <div className="mt-3">
              <p className="text-[10px] text-slate-500 mb-1.5">Known materials:</p>
              <div className="flex flex-wrap gap-1.5">
                {knownMaterials.map((name) => (
                  <button
                    key={name}
                    type="button"
                    onClick={() => { setQuery(name); fetchMaterial(name); }}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-5">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h3 className="text-base font-semibold text-slate-100">{material.name}</h3>
                  <p className="text-xs text-slate-500 mt-0.5">CAS: {material.cas_number}</p>
                </div>
                <span className="text-[10px] text-slate-500">
                  {material.is_conservative ? "Conservative estimate" : `Source: ${material.source}`}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="bg-slate-700/30 rounded-lg p-3 text-center">
                  <div className="text-[10px] text-slate-500 mb-1">Flammable Range</div>
                ...
                <div className="bg-slate-700/30 rounded-lg p-3 text-center">
                  ...
                  <div className="text-sm font-bold text-slate-100 font-mono">{material.flash_point_c}°C</div>
                  <div className="text-[10px] text-slate-500">Flash Point</div>
                </div>
                <div className="bg-slate-700/30 rounded-lg p-3 text-center">
                  <AlertTriangle className="h-3.5 w-3.5 text-red-400 mx-auto mb-1" />
                  <div className="text-sm font-bold text-slate-100 font-mono">{material.auto_ignition_c}°C</div>
                  <div className="text-[10px] text-slate-500">Auto-Ignition</div>
                </div>
                <div className="bg-slate-700/30 rounded-lg p-3 text-center">
                  <Beaker className="h-3.5 w-3.5 text-cyan-400 mx-auto mb-1" />
                  <div className="text-sm font-bold text-slate-100 font-mono">{material.molecular_weight}</div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
                <h4 className="text-xs font-medium text-slate-400 mb-3">Material Group</h4>
                <div className="text-lg font-bold text-cyan-400 font-mono">{material.material_group}</div>
                <p className="text-[10px] text-slate-500 mt-1">Per IEC 60079-0 Table 1</p>
              </div>
              <div className="bg-slate-800/50 border border-slate-700 rounded-xl p-4">
                <h4 className="text-xs font-medium text-slate-400 mb-3">Temperature Class</h4>
                <div className="text-lg font-bold text-cyan-400 font-mono">{material.temperature_class}</div>
                <p className="text-[10px] text-slate-500 mt-1">Per IEC 60079-0</p>
              </div>
            </div>
        ...
        <div className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-4">
          <p className="text-xs text-slate-500">
            Data sourced from internal IEC 60079-10-1 Table B.1 / NFPA 497 database
            (12 common materials) with PubChem API fallback. Conservative defaults
            applied when exact data is unavailable. Temperature class determines
            maximum surface temperature for equipment in hazardous areas.
          </p>
        </div>
      </div>
    </div>

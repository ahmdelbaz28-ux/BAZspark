...
export function IFC43MappingPage() {
	...
	// Results
	const [detectorResult, setDetectorResult] = useState<Record<
		string,
		unknown
	> | null>(null);
	const [projectResult, setProjectResult] = useState<Record<
		string,
		unknown
	> | null>(null);
	...
	const detectorsCount: string = String(
		projectResult?.detectors_count ?? "N/A",
	);
	const schemaVersion: string =
		(projectResult?.schema_version as string) || "N/A";
	const buildingGlobalId: string =
		(projectResult?.building_global_id as string) || "N/A";
						...
						<Card className="border-border bg-card stagger-card">
							<CardHeader className="pb-3">
								...
								<CardDescription className="text-muted-foreground">
									Map a single fire alarm detector to its IFC 4.3 ADD2
									representation
								</CardDescription>
							</CardHeader>
							<CardContent className="space-y-4">
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
									...
									<div className="space-y-2">
										<Label className="text-foreground/90">Occupancy Type</Label>
										<select
											value={occupancyType}
					...
					<TabsContent value="project">
						<Card className="border-border bg-card stagger-card">
							<CardHeader className="pb-3">
								...
								<CardDescription className="text-muted-foreground">
									Map an entire FireAI project (rooms + detectors) to IFC 4.3
									ADD2 schema
								</CardDescription>
							</CardHeader>
							<CardContent className="space-y-4">
								<div className="space-y-2">
									<Label className="text-foreground/90">Project JSON</Label>
									<Textarea
										value={projectJson}
				...
				<Card className="border-border bg-card stagger-card">
					...
					<CardContent>
						<p className="text-sm text-muted-foreground">
							...
							<code className="text-cyan-400 mx-1">
								IfcFireSuppressionTerminal
							</code>
							and predefined types for smoke detectors, heat detectors, and
							notification devices. This mapping translates FireAI's internal
							device model into standards-compliant IFC entities for
							interoperability with BIM authoring tools (Revit, ArchiCAD,
							Solibri).
						</p>
					</CardContent>
				</Card>

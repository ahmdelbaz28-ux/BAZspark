		...
		return (
			<div className="flex flex-col items-center justify-center p-4 bg-[#04060a]/80 border border-[rgba(74,85,104,0.25)] rounded-md shadow-inner">
				...
				<div className="mt-3 text-center">
					<span className="text-[10px] font-bold text-[#c9a84c] font-mono uppercase tracking-wider">
						{label}
					</span>
				</div>
			</div>
...
export function MarinePage() {
	// NOSONAR — S3776: marine page must integrate many sub-systems (viewport, SCADA, logic, alerts, crew)
	...
	// ── API Result States ───────────────────────────────────────────────────
	const [, setStandards] = useState<
		Array<{ code: string; title: string; issuer: string }>
	>([]); // NOSONAR: standards reserved for display
	const [, setFireClasses] = useState<
		Array<{
			class_name: string;
			insulation_minutes: number;
			description: string;
		}>
	>([]); // NOSONAR: fireClasses reserved for display
	...
	const [zones, setZones] = useState<
		Array<{
			...
			name: string;
			area_m2: number;
			required_fire_class: string;
			deck?: string;
		}>
	>([]);
	const [detection, setDetection] = useState<Record<string, unknown> | null>(
		null,
	);
	const [extinguishing, setExtinguishing] = useState<Record<
		string,
		unknown
	> | null>(null);
	const [, setDivisions] = useState<Record<string, unknown> | null>(null); // NOSONAR: divisions reserved for display
	const [alarmLogic, setAlarmLogic] = useState<Record<string, unknown> | null>(
		null,
	);
	const [powerDesign, setPowerDesign] = useState<Record<
		string,
		unknown
	> | null>(null);
	const [scadaConfig, setScadaConfig] = useState<Record<
		string,
		unknown
	> | null>(null);
	...
	const toggleAlarmSimulation = useCallback(() => {
		if (!alarmActive) {
			...
			toast({
				...
				description:
					"Vessel safety systems returned to NORMAL monitoring state.",
			});
		}
	...
	const handleValidate = async () => {
		...
		try {
			...
			toast({
				...
				description:
					err instanceof Error ? err.message : "Failed to run SOLAS validation",
				variant: "destructive",
			});
		} finally {
			setLoading(null);
	...
	const handleDivideZones = async () => {
		...
		try {
			...
			toast({
				...
				description:
					err instanceof Error ? err.message : "Failed to divide vessel zones",
				variant: "destructive",
			});
		} finally {
			setLoading(null);
	...
	const handleRunFullPipeline = async () => {
		...
		try {
			...
			if (data.extinguishing)
				setExtinguishing(data.extinguishing as Record<string, unknown>);
			if (data.detection)
				setDetection(data.detection as Record<string, unknown>);
			if (data.divisions)
				setDivisions(data.divisions as Record<string, unknown>);
			if (data.alarm_logic)
			...
			toast({
				...
				description:
					"Generated complete marine fire protection, divisions, extinguishing & power specifications.",
			});
		} catch (err) {
			toast({
				title: "Design Pipeline Failed",
				description:
					err instanceof Error ? err.message : "Full design pipeline failed",
				variant: "destructive",
			});
		} finally {
			setLoading(null);
	...
	const handleDetection = async () => {
		...
		try {
			...
			toast({
				...
				description:
					err instanceof Error ? err.message : "Failed to size detection",
				variant: "destructive",
			});
		} finally {
			setLoading(null);
			...
			toast({
				...
				description:
					err instanceof Error
						? err.message
						: "Failed to size extinguishing system",
				variant: "destructive",
			});
		} finally {
			setLoading(null);
	...
	const _handleGenerateDivisions = async () => {
		// NOSONAR: reserved for UI button
		...
		try {
			...
			toast({
				...
				description:
					err instanceof Error
						? err.message
						: "Failed to generate fire divisions",
				variant: "destructive",
			});
		} finally {
			setLoading(null);
	...
	const handleGenerateAlarmLogic = async () => {
		...
		try {
			...
			toast({
				...
				description:
					err instanceof Error ? err.message : "Failed to generate alarm logic",
				variant: "destructive",
			});
		} finally {
			setLoading(null);
		...
		try {
			...
			toast({
				...
				description:
					err instanceof Error
						? err.message
						: "Failed to design emergency power",
				variant: "destructive",
			});
		} finally {
			setLoading(null);
	...
	return (
		<div
			className="marine-page flex-1 overflow-auto"
			aria-label={t("nav.marine", "Marine")}
		>
			<div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
				...
				<div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
					<div>
						...
						<p className="marine-label text-[#4a5568] ml-[52px]">
							SOLAS II-2 · IMO FSS Code · IEC 60092 · Classification Rules (LR /
							DNV / ABS)
						</p>
					</div>
					...
					<div className="flex items-center gap-2.5">
						...
						>
							<Siren
								className={`h-4 w-4 mr-1.5 ${alarmActive ? "animate-pulse text-[#e63946]" : "text-[#c9a84c]"}`}
							/>
				...
				<div className="marine-telemetry-bar">
					<div
						className={`marine-telemetry-item ${ship.length_overall_m ? "marine-telemetry-item--accent" : ""}`}
					>
						<div className="marine-telemetry-label">Vessel LOA</div>
						<div className="marine-telemetry-value marine-telemetry-value--brass">
							{ship.length_overall_m} m
						</div>
					</div>
					<div className="marine-telemetry-item">
						<div className="marine-telemetry-label">Gross Tonnage</div>
						<div className="marine-telemetry-value">
					...
					<div className="marine-telemetry-item">
						...
						<div className="marine-telemetry-value">
							{zones.length > 0 ? zones.length : "4 (Auto)"}
						</div>
					</div>
					<div className="marine-telemetry-item">
						...
						<div className="mt-1.5">
							<span
								className={`marine-badge ${simulatedDamperClosed ? "marine-badge--danger" : "marine-badge--safe"}`}
							>
					...
					<div className="marine-telemetry-item">
						...
						<div className="mt-1.5">
							<span
								className={`marine-badge ${simulatedCo2Discharging ? "marine-badge--danger" : "marine-badge--neutral"}`}
							>
				...
				>
					...
					<TabsContent value="viewport" className="space-y-6 m-0">
						...
						<div
							className={`marine-viewport ${alarmActive ? "marine-viewport--alarm" : ""}`}
						>
							<svg
							...
							>
								...
								<text
									x="20"
									y="245"
									fill="rgba(126,200,164,0.4)"
									fontSize="10"
									fontFamily="monospace"
								>
									WATERLINE (DRAFT 7.2m)
								</text>
						...
						<div className="p-4 rounded-md bg-[rgba(4,6,10,0.8)] border border-[rgba(74,85,104,0.5)] flex flex-col md:flex-row items-center justify-between gap-4">
							...
							<div className="flex items-center gap-2">
								<Button
									data-testid="marine-detection-btn"
									size="sm"
									variant="outline"
									onClick={handleDetection}
									disabled={!!loading}
									className="marine-btn marine-btn--secondary h-8"
								>
									{loading === "detection" ? (
										<Loader2 className="h-3.5 w-3.5 animate-spin mr-1" />
									) : (
										<Zap className="h-3.5 w-3.5 mr-1 text-[#c9a84c]" />
									)}
									Design Zone Detection
								</Button>

								<Button
									data-testid="marine-extinguishing-btn"
									size="sm"
									variant="outline"
									onClick={handleExtinguishing}
									disabled={!!loading}
									className="marine-btn marine-btn--secondary h-8"
								>
									{loading === "extinguishing" ? (
										...
										<Flame className="h-3.5 w-3.5 mr-1 text-[#c9a84c]" />
									)}
									Size Extinguishing
								</Button>
							</div>
						...
						<div className="marine-card">
							<div className="marine-card-header">
								<div className="flex items-center gap-2">
									...
									<h2 className="marine-display text-lg">
										Vessel Specifications & Classification
									</h2>
								</div>
								<p className="text-xs text-[#4a5568] mt-1">
									Enter ship dimensions and registration details for SOLAS
							...
							<div className="p-5">
								<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
									...
									<div className="space-y-1.5">
										...
										<Select
											...
											onValueChange={setClassificationSociety}
										>
											<SelectTrigger className="marine-select-trigger">
												<SelectValue />
											</SelectTrigger>
								...
								<div className="mt-5 flex items-center gap-3">
									<Button
										data-testid="marine-validate-btn"
									...
									<Button
										data-testid="marine-divide-zones-btn"
										onClick={handleDivideZones}
										disabled={loading === "zones"}
										className="marine-btn marine-btn--secondary"
									>
										{loading === "zones" ? (
											<Loader2 className="h-4 w-4 animate-spin mr-1.5" />
										) : (
						...
						{validation && (
							<div className="marine-card">
								<div className="marine-card-header flex flex-row items-center justify-between">
									...
									<span
										className={`marine-badge ${(validation as { compliant?: boolean }).compliant ? "marine-badge--safe" : "marine-badge--danger"}`}
									>
										{(validation as { compliant?: boolean }).compliant
											? "PASS — SOLAS COMPLIANT"
											: "FAIL — NON COMPLIANT"}
									</span>
								</div>
								<div className="p-4">
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
							...
							<div className="marine-card">
								...
								<div className="p-4 space-y-4">
									<Button
										data-testid="marine-calculate-sensor-btn"
										onClick={handleDetection}
										disabled={loading === "detection"}
										className="marine-btn marine-btn--primary w-full"
									>
										{loading === "detection" ? (
									...
									{detection && (
										<div className="space-y-4">
											<AnalogGauge
												value={Number(
													(detection as { placements?: unknown[] })?.placements
														?.length ?? 0,
												)}
												min={0}
												max={50}
											...
											<div className="bg-[#04060a] border border-[rgba(74,85,104,0.3)] rounded-md p-3 font-mono text-[10px] text-[#b0b8c4] space-y-1.5">
												<div className="text-[9px] uppercase tracking-wider text-[#4a5568] font-bold border-b border-[rgba(74,85,104,0.2)] pb-1">
													Layout Summary
												</div>
												{(
													(
												...
												).map((c: Record<string, unknown>, _ci: number) => (
													<div
														key={String(c.detector_type)}
														className="flex justify-between items-center"
													>
														<span className="text-[#c9a84c] uppercase">
															{String(c.detector_type).replace("_", " ")}
														</span>
												...
												<div className="text-[8px] text-[#4a5568] mt-2 leading-tight">
													...
													{(
														detection as {
															selection?: { standard_reference?: string };
														}
													)?.selection?.standard_reference ?? "SOLAS Reg. 12"}
												</div>
											</div>
										</div>
									)}
							...
							<div className="marine-card">
								...
								<div className="p-4 space-y-4">
									<Button
										data-testid="marine-size-extinguishing-btn"
										onClick={handleExtinguishing}
									...
									{extinguishing && (
										<div className="space-y-4">
											<AnalogGauge
												value={Number(
													(extinguishing as { agent_quantity_kg?: number })
														?.agent_quantity_kg ?? 0,
												)}
												min={0}
												max={5000}
											...
											<div className="bg-[#04060a] border border-[rgba(74,85,104,0.3)] rounded-md p-3 font-mono text-[10px] text-[#b0b8c4] space-y-1.5">
												...
												<div className="flex justify-between items-center">
													...
													<span className="uppercase text-[#2ec4b6]">
														{
															(extinguishing as { system_type?: string })
																?.system_type
														}
													</span>
												</div>
												...
												<div className="flex justify-between items-center">
													...
													<span>
														{(extinguishing as { nozzles?: number })?.nozzles ??
															0}{" "}
														/{" "}
														{(extinguishing as { pipe_length_m?: number })
															?.pipe_length_m ?? 0}
														m
													</span>
												</div>
												...
												<div className="flex justify-between items-center">
													...
													<span>
														{
															(extinguishing as { discharge_time_s?: number })
																?.discharge_time_s
														}
														s
													</span>
												...
												<div className="text-[8px] text-[#4a5568] mt-2 leading-tight">
													Reference:{" "}
													{
														(extinguishing as { standard_reference?: string })
															?.standard_reference
													}
												</div>
											</div>
							...
							<div className="marine-card">
								...
								<div className="p-4 space-y-4">
									<Button
										data-testid="marine-design-power-btn"
										onClick={handleDesignPower}
									...
									{powerDesign && (
										<div className="space-y-4">
											<AnalogGauge
												value={Number(
													(powerDesign as { ups_capacity_ah?: number })
														?.ups_capacity_ah ?? 0,
												)}
												min={0}
												max={100}
											...
											<div className="bg-[#04060a] border border-[rgba(74,85,104,0.3)] rounded-md p-3 font-mono text-[10px] text-[#b0b8c4] space-y-1.5">
												<div className="text-[9px] uppercase tracking-wider text-[#4a5568] font-bold border-b border-[rgba(74,85,104,0.2)] pb-1">
													Power Distribution Spec
												</div>
												<div className="flex justify-between items-center">
													<span className="text-[#c9a84c]">Main Supply</span>
													<span>
														{
															(powerDesign as { main_supply_voltage?: number })
																?.main_supply_voltage
														}
														V AC
													</span>
													...
													<span>
														{
															(
																powerDesign as {
																	emergency_supply_voltage?: number;
																}
															)?.emergency_supply_voltage
														}
														V AC
													</span>
												</div>
												<div className="flex justify-between items-center">
													...
													<span>
														{
															(powerDesign as { ups_autonomy_min?: number })
																?.ups_autonomy_min
														}{" "}
														min
													</span>
												...
												<div className="flex justify-between items-center">
													...
													<span className="text-[#2ec4b6]">
														{(
															powerDesign as { insulation_monitoring?: boolean }
												...
												<div className="text-[8px] text-[#4a5568] mt-2 leading-tight">
													Standard compliance:{" "}
													{
														(powerDesign as { standard_reference?: string })
															?.standard_reference
													}
												</div>
											</div>
							...
							<div className="marine-card">
								<div className="marine-card-header flex flex-row items-center justify-between">
									<div>
										...
										<CardDescription className="text-xs text-[#4a5568] mt-1">
											Generates automatic FACP trigger matrix for fire dampers &
											alarms
										</CardDescription>
									</div>
									{alarmLogic && (
								...
								<div className="p-4 space-y-4">
									<div className="flex items-center justify-between gap-3">
										<Button
											data-testid="marine-generate-alarm-logic-btn"
											onClick={handleGenerateAlarmLogic}
											disabled={loading === "alarm-logic"}
											className="marine-btn marine-btn--primary"
										>
											{loading === "alarm-logic" ? (
												<Loader2 className="h-4 w-4 animate-spin mr-2" />
										...
										{alarmLogic && (
											<span className="text-[10px] font-mono text-[#b0b8c4] bg-[#0f172a] px-2 py-1 rounded border border-[rgba(74,85,104,0.3)]">
												Causes:{" "}
												{(alarmLogic as { node_count?: number })?.node_count ??
													0}{" "}
												| Effects: {(() => {
													...
													).forEach((n: Record<string, unknown>) => {
														((n.action_outputs as string[]) || []).forEach(
															(e: string) => {
																effects.add(e);
															},
														);
													...
													<table className="min-w-full text-[11px] font-mono text-[#b0b8c4] border-collapse">
														<thead>
															<tr className="border-b border-[rgba(74,85,104,0.4)] bg-[rgba(15,23,38,0.8)]">
																<th className="p-2 text-left font-bold text-[#f1f5f9] min-w-[140px] max-w-[200px]">
																	Trigger Cause (Input)
																</th>
																<th className="p-2 text-left font-bold text-[#b0b8c4] border-l border-[rgba(74,85,104,0.3)]">
																	Level
																</th>
																<th className="p-2 text-center font-bold text-[#b0b8c4] border-l border-[rgba(74,85,104,0.3)] w-[50px]">
																	Evac
																</th>
																{(() => {
																	const effects = new Set<string>();
																	(
																		...
																		(
																			(n.action_outputs as string[]) || []
																		).forEach((e: string) => effects.add(e));
																	});
																	const uniqueEffects = Array.from(effects);
																	return uniqueEffects.map((eff) => (
																		<th
																			key={eff}
																			className="p-2 text-center font-bold text-[#f1f5f9] border-l border-[rgba(74,85,104,0.3)] min-w-[85px] leading-tight"
																		>
																			{EFFECT_LABELS[eff] ||
																				eff
																					.split("_")
														...
														<tbody>
															{(
																...
																const isSimActive =
																	alarmActive &&
																	node.zone_id ===
																		zones[selectedZoneIndex]?.zone_id;
																const effects = new Set<string>();
																(
																	(
																		alarmLogic as {
																...
																return (
																	...
																	>
																		<td className="p-2 font-semibold text-[#c9a84c] min-w-[140px] max-w-[200px]">
																			...
																			<div
																				className="text-[9px] text-[#4a5568] truncate"
																				title={`${String(node.trigger_detector)} (${String(node.zone_id)})`}
																			>
																				{String(node.trigger_detector)} (
																				{String(node.zone_id)})
																			</div>
																		</td>
																		<td className="p-2 border-l border-[rgba(74,85,104,0.2)]">
																			<span
																				className={`text-[8px] tracking-wide uppercase px-1 py-0.5 rounded font-extrabold ${
																					node.alarm_level === "ACTION"
																						? "bg-[rgba(230,57,70,0.25)] text-[#e63946] border border-[rgba(230,57,70,0.4)]"
																						: node.alarm_level === "ALARM"
																							? "bg-[rgba(245,158,11,0.25)] text-[#f59e0b] border border-[rgba(245,158,11,0.4)]"
																		...
																		<td className="p-2 text-center border-l border-[rgba(74,85,104,0.2)] text-[10px] w-[50px]">
																			{Number(node.delay_s) > 0
																				? `${node.delay_s}s`
																				: "-"}
																		</td>
																		{uniqueEffects.map((eff) => {
																			const isTriggered = (
																				(node.action_outputs as string[]) || []
																			).includes(eff);
																			return (
																				<td
																					key={eff}
																					className="p-2 text-center border-l border-[rgba(74,85,104,0.2)]"
																				>
																					{isTriggered ? (
																						<div className="flex justify-center items-center">
																							<span
																								className={`h-2.5 w-2.5 rounded-full ${isSimActive ? "bg-[#e63946] shadow-[0_0_8px_#e63946] animate-pulse" : "bg-[#c9a84c] shadow-[0_0_3px_rgba(201,168,76,0.5)]"}`}
																							/>
																						</div>
																					) : (
																						<span className="text-[rgba(74,85,104,0.2)] font-light text-[9px]">
																							•
																						</span>
																					)}
																				</td>
														...
														<Button
															...
															onClick={() => {
																navigator.clipboard.writeText(
																	(alarmLogic as { plc_script_st?: string })
																		?.plc_script_st ?? "",
																);
																toast({
																	title: "Copied ST Script",
																	description:
																		"PLC program copied to clipboard successfully.",
																});
															}}
														>
															Copy Script
							...
							<div className="marine-card">
								...
								<div className="p-4 space-y-3">
									<div className="grid grid-cols-2 gap-2.5">
										<Button
											data-testid="marine-export-scada-btn"
											variant="outline"
											onClick={handleExportSCADA}
											disabled={loading === "export-scada"}
											className="marine-btn marine-btn--secondary h-9 justify-start"
										>
											{loading === "export-scada" ? (
												<Loader2 className="h-4 w-4 animate-spin mr-2" />
											) : (
												<Server className="h-4 w-4 mr-2 text-[#c9a84c]" />
											)}
											SCADA Config (MQTT)
										</Button>

										<Button
											data-testid="marine-export-etap-btn"
											variant="outline"
											onClick={handleExportETAP}
											disabled={loading === "export-etap"}
											className="marine-btn marine-btn--secondary h-9 justify-start"
										>
											{loading === "export-etap" ? (
												<Loader2 className="h-4 w-4 animate-spin mr-2" />
											) : (
												<FileSpreadsheet className="h-4 w-4 mr-2 text-[#c9a84c]" />
											)}
											ETAP CSV Export
										</Button>

										<Button
											data-testid="marine-export-dxf-btn"
											variant="outline"
											onClick={handleExportDXF}
											disabled={loading === "export-dxf"}
											className="marine-btn marine-btn--secondary h-9 justify-start"
										>
											{loading === "export-dxf" ? (
												<Loader2 className="h-4 w-4 animate-spin mr-2" />
											) : (
												<FileCode2 className="h-4 w-4 mr-2 text-[#c9a84c]" />
											)}
											AutoCAD DXF Ship Plan
										</Button>

										<Button
											data-testid="marine-export-revit-btn"
											variant="outline"
											...
											{scadaView === "table" ? (
												<div className="overflow-x-auto w-full border border-[rgba(74,85,104,0.3)] rounded-md bg-[#04060a]">
													<table className="min-w-full text-[10px] font-mono text-[#b0b8c4] border-collapse">
														<thead>
															<tr className="border-b border-[rgba(74,85,104,0.4)] bg-[rgba(15,23,38,0.8)]">
																<th className="p-2 text-left font-bold text-[#f1f5f9]">
																	Tag ID
																</th>
																<th className="p-2 text-left font-bold text-[#b0b8c4] border-l border-[rgba(74,85,104,0.3)]">
																	Description
																</th>
																<th className="p-2 text-left font-bold text-[#f1f5f9] border-l border-[rgba(74,85,104,0.3)]">
																	MQTT Topic Address
																</th>
																<th className="p-2 text-center font-bold text-[#b0b8c4] border-l border-[rgba(74,85,104,0.3)] w-[60px]">
																	Format
																</th>
															</tr>
														</thead>
														<tbody>
															{(
																...
																<tr
																	...
																	className="border-b border-[rgba(74,85,104,0.2)] hover:bg-[rgba(201,168,76,0.04)]"
																>
																	<td className="p-2 font-semibold text-[#c9a84c] truncate max-w-[150px]">
																		{String(tag.tag_id)}
																	</td>
																	<td className="p-2 border-l border-[rgba(74,85,104,0.2)]">
																	...
																	<td
																		className="p-2 border-l border-[rgba(74,85,104,0.2)] text-[#2ec4b6] truncate max-w-[250px]"
																		title={String(tag.address)}
																	>
																		{String(tag.address)}
																	</td>
												...
												<div className="space-y-2">
													<div className="flex items-center justify-between">
														...
														<Button
															...
															onClick={() => {
																navigator.clipboard.writeText(
																	(scadaConfig as { pyscada_yaml?: string })
																		?.pyscada_yaml ?? "",
																);
																toast({
																	title: "Copied YAML Configuration",
																	description:
																		"SCADA config YAML copied to clipboard successfully.",
																});
															}}
														>
															Copy YAML
				...
				<div className="marine-section mt-6">
					...
					<div className="flex flex-wrap gap-3 mt-3">
						<Button
							...
							onClick={async () => {
								if (!imoNumber) {
									toast({ title: "Ship ID required", variant: "destructive" });
									return;
								}
								setLoading("alarm-logic");
								try {
									const res = await marineExtendedApi.generateAlarmLogic({
										ship_id: imoNumber,
									});
									setAlarmLogic(res as Record<string, unknown>);
						...
						<Button
							...
							onClick={async () => {
								...
								setLoading("divisions");
								try {
									const res = await marineExtendedApi.generateDivisions({
										ship_id: imoNumber,
									});
									setDivisions(res as Record<string, unknown>);
						...
						<Button
							...
							onClick={async () => {
								...
								try {
									const res = await marineExtendedApi.generateScada({
										ship_id: imoNumber,
									});
									setScadaConfig(res as Record<string, unknown>);

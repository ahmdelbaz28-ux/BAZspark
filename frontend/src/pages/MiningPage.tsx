...
export function MiningPage() {
	...
	const [methaneResult, setMethaneResult] = useState<Record<
		string,
		unknown
	> | null>(null);
	...
	const [ventResult, setVentResult] = useState<Record<string, unknown> | null>(
		null,
	);

	// CO state
	const [coPpm, setCoPpm] = useState("10");
	const [coResult, setCoResult] = useState<Record<string, unknown> | null>(
		null,
	);
	...
	const [conveyorResult, setConveyorResult] = useState<Record<
		string,
		unknown
	> | null>(null);
	...
	// Standards state
	const [standardsResult, setStandardsResult] = useState<Array<{
		code: string;
		title: string;
	}> | null>(null);
	...
	const handleReport = async () => {
		...
		try {
			...
			toast.error(
				`Report failed: ${err instanceof Error ? err.message : "Unknown"}`,
			);
		} finally {
			setLoading(false);
		}
			...
			{activeTab === "standards" && (
				<Card className="border-border bg-card stagger-card">
					<CardHeader>
						...
						<CardDescription>
							NFPA 120/122 · MSHA 30 CFR · IEC 60079
						</CardDescription>
					</CardHeader>
			...
			{activeTab === "ventilation" && (
				<Card className="border-border bg-card stagger-card">
					<CardHeader>
						...
						<CardDescription>
							MSHA 30 CFR §75.326-327 — Minimum Airflow Requirements
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
							...
							<div className="space-y-2">
								...
								<Select value={ventLocation} onValueChange={setVentLocation}>
									...
									<SelectContent>
										...
										<SelectItem value="last_open_crosscut">
											Last Open Crosscut
										</SelectItem>
			...
			{activeTab === "co" && (
				<Card className="border-border bg-card stagger-card">
					<CardHeader>
						...
						<CardDescription>
							MSHA 30 CFR §75.351 — CO Monitoring at Belt Entries
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						<div className="flex items-end gap-3">
						...
						{coResult && (
							<div className="space-y-2 p-4 rounded-lg bg-muted/50">
								...
								<div className="text-sm text-muted-foreground space-y-1">
									<p>
										Alert:{" "}
										{(coResult.thresholds as Record<string, number>)?.alert_ppm}{" "}
										ppm
									</p>
									<p>
										Evacuate:{" "}
										{
											(coResult.thresholds as Record<string, number>)
												?.evacuate_ppm
										}{" "}
										ppm
									</p>
			...
			{activeTab === "conveyor" && (
				<Card className="border-border bg-card stagger-card">
					<CardHeader>
						...
						<CardDescription>
							NFPA 120-2022 §8.4 + MSHA 30 CFR §75.1108
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						<div className="grid grid-cols-2 gap-4">
								...
								{(conveyorResult.design as Record<string, unknown>) && (
									<>
										<p className="text-sm">
											...
											{
												(conveyorResult.design as Record<string, unknown>)
													.number_of_nozzle_groups as number
											}
										</p>
										<p className="text-sm">
											Water Flow:{" "}
										...
										<p className="text-sm">
											...
											{
												(conveyorResult.design as Record<string, unknown>)
													.total_water_volume_l as number
											}{" "}
											L
										</p>
										<p className="text-sm">
			...
			{activeTab === "report" && (
				<Card className="border-border bg-card stagger-card">
					<CardHeader>
						...
						<CardDescription>
							Aggregated compliance check for mine section
						</CardDescription>
					</CardHeader>
					<CardContent className="space-y-4">
						<div className="grid grid-cols-2 gap-4">
						...
						{reportResult && (
							<div className="space-y-2 p-4 rounded-lg bg-muted/50">
								<Badge
									className={
										reportResult.overall_status === "PASS"
											? "bg-emerald-600"
											: reportResult.overall_status === "WARNING"
												? "bg-amber-500"
												: "bg-red-600"
									}
								>
								...
								{(reportResult.checks as Array<Record<string, string>>)?.map(
									(check, i) => (
										<div
											key={i}
											className="text-sm border-l-2 border-border pl-3 mt-2"
										>
								...
								{reportResult.markdown_report ? (
									<Button
										...
										onClick={() => {
											const blob = new Blob(
												[reportResult.markdown_report as string],
												{ type: "text/markdown" },
											);

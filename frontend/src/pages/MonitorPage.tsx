...
function getMonitorWsUrl(): string {
	...
	return (
		apiBase.replace(/^http/, "ws").replace(/\/api\/v\d+/, "/ws") + "/monitor"
	);
}
		...
		for (const msg of messages) {
			switch (msg.type) {
				...
				case "engine_status":
					...
					break;
				case "agent_activity": {
					const activities = (msg.data as { activities?: unknown[] })
						.activities;
				...
				<div className="flex items-center justify-between">
					<div>
						...
						<p className="text-sm text-muted-foreground mt-1">
							Engine health · Agent activity · Security alerts · Prometheus
							metrics
						</p>
					</div>

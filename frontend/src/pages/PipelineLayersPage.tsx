...
export function PipelineLayersPage() {
	...
	const runPipeline = async () => {
		setRunning(true);
		setFinalResult(null);
		const updatedLayers = LAYER_NAMES.map((name) => ({
			name,
		...
		for (let i = 0; i < LAYER_NAMES.length; i++) {
			setLayers((prev) =>
				prev.map((l, idx) =>
					idx === i ? { ...l, status: "running" as const } : l,
				),
			);
			try {
				...
				setLayers((prev) =>
					prev.map((l, idx) =>
						idx === i ? { ...l, status: "failed" as const } : l,
					),
				);
				break;
			}
			...
			<div className="flex items-center gap-3">
				...
				<div>
					...
					<p className="text-muted-foreground">
						5+1 layer NFPA 72 compliance pipeline
					</p>
				</div>
			</div>
			...
			<div className="flex gap-4 items-end">
				<div className="space-y-2">
					<label className="text-sm font-medium">
						{t("fireai.pipeline.regulatoryFramework")}
					</label>
					<Select value={framework} onValueChange={setFramework}>
						...
						<SelectContent>
							<SelectItem value="nfpa72">
								{t("fireai.pipeline.nfpa72")}
							</SelectItem>
							<SelectItem value="nec2023">
								{t("fireai.pipeline.nec2023")}
							</SelectItem>
							<SelectItem value="ibc2021">
								{t("fireai.pipeline.ibc2021")}
							</SelectItem>
						</SelectContent>
					</Select>
				</div>

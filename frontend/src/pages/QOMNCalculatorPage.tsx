...
function useServerVerify() {
	...
	const verify = async (label: string, call: () => Promise<unknown>) => {
		...
		try {
			...
			setState({
				loading: false,
				result: resp as Record<string, unknown>,
				error: null,
			});
			toast({
			...
			const msg =
				err instanceof Error ? err.message : "Server verification failed";
 ...
 */
const ServerVerifyPanel: React.FC<{ state: ServerVerifyState }> = ({
	state,
}) => {
	if (state.loading) {
		return (
	...
	if (state.error) {
		return (
			<div className="baz-panel p-4 border border-red-500/30">
				...
				<pre className="text-[11px] font-mono text-red-300/80 whitespace-pre-wrap">
					{state.error}
				</pre>
			</div>
		);
...
const FieldLabel: React.FC<{
	...
	unit?: string;
	htmlFor?: string;
}> = ({ children, unit, htmlFor }) => (
	<label
		htmlFor={htmlFor}
		className="flex items-baseline justify-between mb-1.5"
	>
		<span className="text-[11px] font-medium text-[#6a6a80] uppercase tracking-wider">
			{children}
		</span>
		{unit && (
			<span className="text-[10px] text-[#3a3a50] font-mono">{unit}</span>
		)}
	</label>
);
...
const MetricCard: React.FC<MetricProps> = ({
	...
	return (
		<div
			className={`baz-panel p-4 border ${statusColors[status].split(" ")[0]}`}
		>
			<div className="flex items-start justify-between gap-2 mb-3">
				...
				{StatusIcon && (
					<StatusIcon
						size={13}
						className={statusColors[status].split(" ")[1]}
						strokeWidth={2}
					/>
				)}
			</div>
	...
	const guards: GuardRule[] = [
		{
			...
			description: "NFPA 72 Table 23.3.6 — max 30 ft",
			severity: "error",
			category: "spacing",
			min: 20,
			max: 30,
		...
		},
		{
			id: "s2",
			name: "Ceiling Height",
			description: "Standard detectors: 8–12 ft",
			severity: "error",
			category: "smoke",
			min: 8,
			max: 50,
		...
		},
		{
			id: "s3",
			name: "Coverage Area",
			description: `${requiredDetectors} detectors × 900 sq ft`,
			severity: "warn",
			category: "smoke",
			status: "pass",
		},
	...
	return (
		<div className="space-y-6 anim-fade-in">
			<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
				<div>
					<FieldLabel htmlFor="smoke-room-area" unit="sq ft">
						Room Area
					</FieldLabel>
					<NumInput
						id="smoke-room-area"
				...
				<div>
					<FieldLabel htmlFor="smoke-ceiling-height" unit="ft">
						Ceiling Height
					</FieldLabel>
					<NumInput
						id="smoke-ceiling-height"
						value={ceilingHeight}
			...
			<div className="grid grid-cols-2 gap-3">
				<MetricCard
					label="Required Detectors"
					value={requiredDetectors}
					unit={`for ${roomArea.toLocaleString()} sq ft`}
					status="neutral"
				/>
				<MetricCard
					label="Max Spacing"
...
const BatteryCalculator: React.FC = () => {
	...
	const guards: GuardRule[] = [
		{
			id: "b1",
			name: "Alarm Duration Min.",
			description: "NFPA 72: Min 5 min alarm",
			severity: "error",
			category: "battery",
			min: 5,
			max: 60,
		...
		},
		{
			...
			description: "Required 20% safety factor",
			severity: "warn",
			category: "battery",
			status: "pass",
		},
		{
			id: "b3",
			name: "Total w/ Margin",
			description: `${marginAh.toFixed(2)} Ah required`,
			severity: "warn",
			category: "battery",
			status: "pass",
		},
	...
	return (
		<div className="space-y-6 anim-fade-in">
			<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
				...
				<div>
					<FieldLabel htmlFor="battery-current-draw" unit="mA">
						Current Draw
					</FieldLabel>
					<NumInput
						id="battery-current-draw"
						value={currentDraw}
				...
				<div>
					<FieldLabel htmlFor="battery-standby-hours" unit="h">
						Standby Hours
					</FieldLabel>
					<NumInput
						id="battery-standby-hours"
						value={standbyHours}
				...
				<div>
					<FieldLabel htmlFor="battery-alarm-minutes" unit="min">
						Alarm Duration
					</FieldLabel>
					<NumInput
						id="battery-alarm-minutes"
						value={alarmMinutes}
...
const VoltageDropCalculator: React.FC = () => {
	...
	const guards: GuardRule[] = [
		{
			id: "v1",
			name: "Voltage Drop",
			description: "NFPA 72: Max 5% drop allowed",
			severity: "error",
			category: "voltage",
			min: 0,
			max: 5,
			currentValue: pctDrop,
			unit: "%",
			status: voltStatus,
		},
	];
	...
	return (
		<div className="space-y-6 anim-fade-in">
			<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
				<div>
					<FieldLabel htmlFor="voltage-wire-length" unit="ft">
						Wire Length
					</FieldLabel>
					<NumInput
						id="voltage-wire-length"
						value={wireLength}
				...
				<div>
					<FieldLabel htmlFor="voltage-wire-gauge" unit="AWG">
						Wire Gauge
					</FieldLabel>
					<SelectInput
						id="voltage-wire-gauge"
						value={String(wireGauge)}
						onChange={(v) => setWireGauge(Number(v))}
						options={[
							{ value: "8", label: "#8 AWG" },
				...
				<div>
					<FieldLabel htmlFor="voltage-current" unit="A">
						Current
					</FieldLabel>
					<NumInput
						id="voltage-current"
			...
			<div className="grid grid-cols-2 gap-3">
				...
				<MetricCard
					...
					value={pctDrop}
					precision={1}
					unit={`of 12V — ${pctDrop <= 5 ? "within limit" : "exceeds limit"}`}
					status={voltStatus}
				/>
			</div>
	...
	return (
		<div className="space-y-6 anim-fade-in">
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div>
					<FieldLabel htmlFor="heat-ceiling-height" unit="ft">
						Ceiling Height
					</FieldLabel>
					<NumInput
						id="heat-ceiling-height"
						value={ceilingHeight}
						onChange={setCeilingHeight}
						min={6}
						max={30}
				...
				<div>
					<FieldLabel htmlFor="heat-area-per-detector" unit="sq ft">
						Coverage Area per Detector
					</FieldLabel>
					<NumInput
						id="heat-area-per-detector"
						value={areaPerDetector}
						onChange={setAreaPerDetector}
						min={100}
						max={2500}
						step={50}
					/>
			...
			<div className="grid grid-cols-2 gap-3">
				<MetricCard
					...
					value={linearSpacingFt}
					precision={1}
					unit="ft (square root of area)"
					status={spacingStatus}
				/>
				<MetricCard
					label="Coverage Area"
					value={areaPerDetector}
					unit="sq ft per detector"
					status="neutral"
				/>
			</div>
...
const DuctDetectorSection: React.FC = () => {
	...
	const [ductResult, setDuctResult] = useState<Record<string, unknown> | null>(
		null,
	);
	...
	const handlePlaceDuct = async () => {
		...
		try {
			...
			toast({
				title: "Duct detector placed",
				description: `Width: ${ductWidth}m`,
			});
		} catch (err) {
			const msg = err instanceof Error ? err.message : "Failed";
			setDuctError(msg);
			toast({
				title: "Place duct failed",
				description: msg,
				variant: "destructive",
			});
		} finally {
	...
	return (
		<div className="space-y-6">
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				<div>
					<FieldLabel htmlFor="duct-width" unit="m">
						Duct Width
					</FieldLabel>
					<NumInput
						id="duct-width"
						value={ductWidth}
						onChange={setDuctWidth}
						min={0.1}
						max={10}
						step={0.1}
					/>
...
export const QOMNCalculatorPage: React.FC = () => {
	...
	const [goldenTestsResult, setGoldenTestsResult] = useState<Record<
		string,
		unknown
	> | null>(null);
	const { toast } = useToast();
		...
		<div className="flex flex-col h-full min-h-0 bg-[#09090d]">
			...
			<div className="shrink-0 px-6 pt-6 pb-4 border-b border-[#131318]">
				<div className="flex items-start justify-between gap-4">
					<div>
						<h1 className="text-[18px] font-semibold text-[#f0f0f2] tracking-tight">
							QOMN Calculator
						</h1>
			...
			<div className="flex-1 overflow-y-auto p-6">
				...
				{activeTab === "detectors" && <PlaceDetectorsSection />}
				{activeTab === "guards" && (
					<div className="space-y-6 anim-fade-in">
						<QomnReadOnlySection
...
const QomnReadOnlySection: React.FC<{
	title: string;
...
const PlaceDetectorsSection: React.FC = () => {
	...
	return (
		<div className="space-y-6 anim-fade-in">
			<div className="baz-card">
				<h3 className="baz-label mb-4">
					Place Detectors (POST /qomn/place-detectors)
				</h3>
				<div className="grid grid-cols-3 gap-4 mb-4">
					<div>
						<label htmlFor="place-room-area" className="baz-label text-[10px]">
							Room Area (m²)
						</label>
						<input
							id="place-room-area"
					...
					<div>
						<label
							htmlFor="place-ceiling-height"
							className="baz-label text-[10px]"
						>
							Ceiling Height (m)
						</label>
						<input
			...
			<div className="baz-card">
				<h3 className="baz-label mb-2 flex items-center gap-2">
					...
					Acoustic & UGLD Raytracing Integration Engine (NFPA 72 §18.4 /
					ISA-TR84.00.07)
				</h3>
				<p className="text-[11px] text-[#7a7a8a] mb-4">
					Evaluates unified audible notification sound propagation and
					ultrasonic gas leak detection raytracing with Maekawa barrier
					diffraction.
				</p>
				<div className="grid grid-cols-3 gap-4 mb-4">
					<div>
						<label
							htmlFor="acoustics-speaker-spl"
							className="baz-label text-[10px]"
						>
							Speaker Rated SPL (dBA @ 3m)
						</label>
						<input
							type="number"
							defaultValue={95}
					...
					<div>
						<label
							htmlFor="acoustics-checkpoint-dist"
							className="baz-label text-[10px]"
						>
							Check Point Distance (m)
						</label>
						<input
							type="number"
							defaultValue={5.0}
					...
					<div>
						<label htmlFor="acoustics-mode" className="baz-label text-[10px]">
							Audible Mode
						</label>
						<select
							id="acoustics-mode"
					...
					onClick={async () => {
						try {
							...
							const dist = Number(
								(
									document.getElementById(
										"acoustics-checkpoint-dist",
									) as HTMLInputElement
								)?.value || 5,
							);
							const mode =
								(document.getElementById("acoustics-mode") as HTMLSelectElement)
									?.value || "public";

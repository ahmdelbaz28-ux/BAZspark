...
export function RbacPage() {
	...
	const [categories, setCategories] =
		useState<PermissionCategory[]>(FALLBACK_CATEGORIES);
	...
	const fetchPermissions = async () => {
		...
		try {
			...
			if (payload.categories)
				setCategories(payload.categories as PermissionCategory[]);
			if (payload.role_permissions)
				setRolePermissions(payload.role_permissions as RolePermissionMap);
		} catch (err) {
										...
										{ROLES.map((role) => (
											<TableHead key={role} className="text-center w-[120px]">
												<div className="flex flex-col items-center gap-1">
													<Badge className={`${roleBadgeClass(role)} text-xs`}>
														{roleLabel(role)}
													</Badge>
												</div>
											</TableHead>

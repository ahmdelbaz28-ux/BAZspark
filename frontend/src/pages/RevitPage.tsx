...
export function RevitPage({ initialTab }: RevitPageProps = {}) {
	...
	const [apiSearchResult, setApiSearchResult] = useState<Record<
		string,
		unknown
	> | null>(null);
	const [nlCommand, setNlCommand] = useState("");
	const [nlResult, setNlResult] = useState<Record<string, unknown> | null>(
		null,
	);

	// Revit Integration state
	const [integrationProjectId, setIntegrationProjectId] = useState("");
	const [integrationResult, setIntegrationResult] = useState<Record<
		string,
		unknown
	> | null>(null);
			...
			<div className="flex items-center justify-between">
				...
				>
					{connected ? (
						...
						<>
							<WifiOff aria-hidden="true" className="h-3 w-3 mr-1" />{" "}
							Disconnected
						</>
					)}
				</Badge>
				...
				>
					...
					<div className="space-y-1">
						...
						<p className="text-xs text-amber-200">
							create_wall/create_floor/create_door will return None. read_rvt
							will return empty results (RVT is a closed format requiring Revit
							API). write_rvt will write a real IFC4 file instead (Revit can
							import via File → Open → IFC). For real Revit integration, use

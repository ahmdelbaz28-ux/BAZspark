...
export function TopologyPage() {
	...
	const [healthStatus, setHealthStatus] = useState<Record<
		...
		unknown
	> | null>(null);
	const [impactResult, setImpactResult] = useState<Record<
		string,
		unknown
	> | null>(null);
	...
	const handleHealth = async () => {
		...
		try {
			...
			toast({
				...
				description: "Topology service status retrieved",
			});
		} catch (err) {
			toast({
				...
				description:
					err instanceof Error
						? err.message
						: "Topology service may not be configured",
				variant: "destructive",
			});
		} finally {
			setLoading(false);
	...
	const handleAddElement = async () => {
		...
		try {
			...
			toast({
				...
				variant: data.added ? "default" : "destructive",
			});
			setElementId("");
			setElementName("");
		} catch (err) {
	...
	const handleAddConnection = async () => {
		if (!fromElement || !toElement) {
			toast({
				title: "Both source and target elements are required",
				variant: "destructive",
			});
			return;
		}
					...
					<TabsContent value="elements">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Add a bus, line, transformer, load, breaker, or generator to
									the topology graph
								</CardDescription>
							</CardHeader>
							<CardContent>
					...
					<TabsContent value="connections">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Define relationships between network elements in the Neo4j
									topology graph
								</CardDescription>
							</CardHeader>
							<CardContent>
					...
					<TabsContent value="impact">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Analyze which loads and buses are affected when a specific
									breaker trips
								</CardDescription>
							</CardHeader>
							<CardContent>
						...
						<CardContent className="py-12">
							<div className="flex flex-col items-center text-center">
								...
								<p className="text-sm text-muted-foreground/60 mt-1">
									Add power system elements and connections, then analyze
									breaker trip impacts
								</p>
							</div>
						</CardContent>
					</Card>

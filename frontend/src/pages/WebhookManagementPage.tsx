...
export function WebhookManagementPage() {
  ...
  const [subEventTypes, setSubEventTypes] = useState<string[]>(["project.updated"]);
  ...
  const handleListSubscriptions = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: err instanceof Error ? err.message : "Webhook service may not be configured",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      toast({ title: "Subscribed", description: `Webhook registered for ${subUrl}` });
      setSubUrl("");
      setSubSecret("");
  ...
  const handleUnsubscribe = async (subId: string) => {
    ...
    try {
      ...
      toast({ title: "Unsubscribed", description: `Subscription ${subId} removed` });
      handleListSubscriptions();
    } catch (err) {
      toast({
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      toast({ title: "Event published", description: `Event type: ${pubEventType}` });
    } catch (err) {
      toast({
    ...
    <div className="flex-1 overflow-auto">
      <div className="p-6 max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          <Button onClick={handleListSubscriptions} disabled={loading} variant="outline">
            {loading ? (
        ...
        {subscriptions.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Currently registered webhook endpoints</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-1.5">
                <Label className="text-xs text-muted-foreground">Event Data (JSON)</Label>
                <Input
                  autoComplete="off"
                  value={pubData}
          ...
          <Card>
            <CardContent className="py-12">
              <div className="flex flex-col items-center text-center">
                ...
                />
                <p className="text-muted-foreground font-medium">No webhook subscriptions</p>

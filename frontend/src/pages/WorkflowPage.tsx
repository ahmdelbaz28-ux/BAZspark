...
export function WorkflowPage() {
	...
	const [globalStatus, setGlobalStatus] = useState<Record<
		...
		unknown
	> | null>(null);
	const [activeWorkflow, setActiveWorkflow] = useState<WorkflowStatus | null>(
		null,
	);
			...
			toast({
				...
				description:
					err instanceof Error
						? err.message
						: "Workflow engine may not be configured",
				variant: "destructive",
			});
		} finally {
			setLoading(false);
	...
	const handleReject = async () => {
		...
		setLoading(true);
		try {
			await workflowApi.reject(workflowId, {
				reason: rejectReason || "Rejected by engineer",
			});
			toast({
				title: "Rejected",
				description: "Workflow step rejected.",
				variant: "destructive",
			});
			setRejectReason("");
			handleGetStatus();
						...
						<div className="flex items-center gap-2">
							{Object.entries(globalStatus).map(([key, val]) => (
								<Badge
									key={key}
									variant={
										val === true || val === "running" ? "default" : "secondary"
									}
									className="text-xs"
								>
									{key}: {String(val)}
						...
						{activeWorkflow && (
							<div className="space-y-3">
								<div className="flex items-center gap-3">
									...
									<Badge
										variant={
											STATUS_VARIANTS[activeWorkflow.status] || "secondary"
										}
									>
									...
									<div className="space-y-2">
										...
										{activeWorkflow.steps.map((step, i) => (
											...
											>
												...
												<Badge
													variant={STATUS_VARIANTS[step.status] || "secondary"}
													className="text-xs"
												>
													{step.status}
												</Badge>

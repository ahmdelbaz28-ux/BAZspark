...
vi.mock("@/components/ui/button", () => ({
  Button: ({ children, onClick, disabled, ...props }: { children: React.ReactNode; onClick?: () => void; disabled?: boolean; variant?: string }) => (
...
vi.mock("@/components/ui/label", () => ({
  Label: ({ children, htmlFor, ...props }: { children: React.ReactNode; htmlFor?: string; className?: string }) => (
    <label htmlFor={htmlFor} {...props}>{children}</label>
  ),
}));

vi.mock("@/components/ui/checkbox", () => ({
  Checkbox: ({ id, checked, onCheckedChange }: { id: string; checked: boolean; onCheckedChange?: (v: boolean | string) => void }) => (
    <input
      type="checkbox"
      id={id}
      checked={checked}
...
const MOCK_PANELS = [
  { model: "FACP-2000", manufacturer: "Notifier", device_capacity: 128, nac_capacity: 4 },
  { model: "FACP-5000", manufacturer: "Notifier", device_capacity: 256, nac_capacity: 8 },
  { model: "FACP-9000", manufacturer: "Honeywell", device_capacity: 512, nac_capacity: 16 },
];

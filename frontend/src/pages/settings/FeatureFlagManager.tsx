		...
		<div className="space-y-6 max-w-4xl mx-auto py-8">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-slate-400">
						Manage system capabilities and experimental features.
					</p>
				</div>
				<Button
					onClick={saveFlags}
			...
			<Card className="border-slate-800 bg-slate-900/50 backdrop-blur-xl">
				<CardHeader>
					...
					<CardDescription>
						Features that directly affect NFPA 72 compliance and deterministic
						results.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-6">
					<div className="flex items-center justify-between p-4 rounded-lg border border-slate-800 bg-slate-900/80">
						<div className="space-y-1">
							...
							<p className="text-sm text-slate-400">
								CFD smoke spread simulation. Produces non-reproducible results
								violating audit trails.
							</p>
						</div>
						<Switch
					...
					<div className="flex items-center justify-between p-4 rounded-lg border border-slate-800 bg-slate-900/80">
						<div className="space-y-1">
							...
							<p className="text-sm text-slate-400">
								ML-based pattern learning. Learned patterns cannot be verified
								against NFPA 72.
							</p>
						</div>
						<Switch
			...
			<Card className="border-slate-800 bg-slate-900/50 backdrop-blur-xl">
				<CardHeader>
					<CardTitle className="text-xl">
						Integration & Analysis Flags
					</CardTitle>
					<CardDescription>
						Standard system capabilities and third-party bridges.
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					{Object.entries(flags)
						.filter(
							([key]) => key !== "SMOKE_SIMULATION" && key !== "SELF_LEARNING",
						)
						.map(([key, value]) => (
							<div
								key={key}
								className="flex items-center justify-between p-4 rounded-lg border border-slate-800 bg-slate-900/40 hover:bg-slate-800/60 transition-colors"
							>
								<div className="space-y-1">
									<span className="font-semibold text-white">{key}</span>
									<p className="text-sm text-slate-500">
										Toggle {key.toLowerCase().replace("_", " ")} capabilities.
									</p>
								</div>
								<Switch
									checked={value}

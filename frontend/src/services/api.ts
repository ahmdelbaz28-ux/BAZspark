...
class ApiClient extends BaseApiClient {
	...
	async getElements(
		...
		return this.get<UdmPaginatedData<Element>>(`/elements${querySuffix}`);
	}
	...
	async getConflicts(
		...
		const query = searchParams.toString();
		const querySuffix = query ? `?${query}` : "";
		return this.get<UdmPaginatedData<Conflict>>(`/conflicts${querySuffix}`);
	}
	...
	async resolveConflict(id: string, strategy: string): Promise<Conflict> {
		return this.post<Conflict>(`/conflicts/${encodeURIComponent(id)}/resolve`, {
			strategy,
		});
	}

...
export type TxnStatus = "PENDING" | "SUCCESS" | "FAILED" | "EXPIRED" | "CANCELLED";
...
export async function listOrders(params?: {
    ...
    return apiCall<Order[]>(`/billing/orders${query ? "?" + query : ""}`, { method: "GET" });
}

export async function getOrder(orderId: string): Promise<Order> {
    return apiCall<Order>(`/billing/orders/${encodeURIComponent(orderId)}`, { method: "GET" });
}
...
export async function listTransactionsForOrder(orderId: string): Promise<PaymentTransaction[]> {
...
export async function listEventsForOrder(orderId: string): Promise<PaymentEvent[]> {
...
export async function getTransaction(txnId: string): Promise<PaymentTransaction> {
    return apiCall<PaymentTransaction>(`/billing/transactions/${encodeURIComponent(txnId)}`, {
        method: "GET",
    });
}
...
export function orderStatusLabel(status: OrderStatus): { en: string; ar: string } {

...
if (
	...
	console.error(
		"[FireAI] SECURITY: Blocking insecure ws:// WebSocket on HTTPS page. Set VITE_WS_URL to wss://.",
	);
}
...
export class DataService {
			...
			this.ws.onopen = () => {
				...
				if (this.apiKey) {
					...
					if (isSecure) {
						...
						console.error(
							"[FireAI] SECURITY: Refusing to send API key over unencrypted WebSocket.",
						);
	...
	private handleWebSocketMessage(message: Record<string, unknown>) {
		...
		if (
			message.type === "fault" ||
			(message.data as Record<string, unknown>)?.fault
		) {
			const fault =
				(message.data as Record<string, unknown>)?.fault || message.data;
	...
	private handleData = (data: Record<string, unknown>) => {
		...
		if (data.fault) {
			...
			actions.addLog(
				`CRITICAL: Server reported fault on ${JSON.stringify(data.fault)}`,
			);
		}
	};

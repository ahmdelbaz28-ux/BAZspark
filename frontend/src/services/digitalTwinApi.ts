...
class DigitalTwinApiClient extends BaseApiClient {
	...
	private readonly wsCallbacks: Map<string, Set<(data: unknown) => void>> =
		new Map();
	...
	async syncProject(projectId: string): Promise<ApiResponse<SyncStatus>> {
		return this.wrapResult(
			this.post<SyncStatus>(`/projects/${encodeURIComponent(projectId)}/sync`),
		);
	}

	async getSyncStatus(projectId: string): Promise<ApiResponse<SyncStatus>> {
		return this.wrapResult(
			this.get<SyncStatus>(`/projects/${encodeURIComponent(projectId)}/sync`),
		);
	}

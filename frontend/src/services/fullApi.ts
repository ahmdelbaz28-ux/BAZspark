...
import { CSRF_HEADER_NAME, getCachedCsrfToken, getCsrfToken } from "./csrf";
...
export const qomnApi = {
	...
	getPhysicsGuards: () => apiCall("/qomn/physics-guards", { method: "GET" }),
	...
	getConstants: () => apiCall("/qomn/constants", { method: "GET" }),

	/** GET /qomn/audit — Export the QOMN audit log (AHJ access) */
	getAudit: () => apiCall("/qomn/audit", { method: "GET" }),
};
...
export const v2Api = {
	...
	setRedisValue: (key: string, value: string, ttl?: number) => {
		...
		return apiCall(
			`/multi-db/redis/set?key=${key}&value=${encodeURIComponent(value)}${ttlParam}`,
			{ method: "POST" },
		);
	},
	...
	executeNeo4jQuery: (queryType: string, parameters?: string) => {
		const paramsSuffix = parameters
			? `&parameters=${encodeURIComponent(parameters)}`
			: "";
		return apiCall(
			`/multi-db/neo4j/query?query_type=${queryType}${paramsSuffix}`,
		);
	},
	...
	findRelatedElements: (
		...
		apiCall(
			`/multi-db/bim/related-elements/${elementId}?relationship_type=${relationshipType}`,
		),
...
export const etapApi = {
	...
	testConnection: async (
		...
		projectId?: string,
	): Promise<EtapConnectionTestResponse> => {
	...
	getStatus: async (
		...
		apiCall(
			`/integrations/etap/status?project_id=${encodeURIComponent(projectId)}`,
		),

	/** GET /integrations/etap/projects */
	listEtapProjects: async (projectId: string): Promise<EtapProjectInfo[]> =>
		apiCall(
			`/integrations/etap/projects?project_id=${encodeURIComponent(projectId)}`,
		),
	...
	importFromEtap: async (
		data: EtapImportRequest,
	): Promise<{
		project_id: string;
		etap_project_id: string;
	...
	getLogs: async (
		...
		page = 1,
		pageSize = 50,
	): Promise<EtapSyncLogResponse> =>
		apiCall(
			`/integrations/etap/logs?project_id=${encodeURIComponent(projectId)}&page=${page}&page_size=${pageSize}`,
		),
	...
	createSettings: async (
		projectId: string,
		settings: EtapConnectionSettings,
	): Promise<EtapSettingsResponse> =>
		apiCall(
			`/integrations/etap/settings?project_id=${encodeURIComponent(projectId)}`,
			{
				method: "POST",
				body: JSON.stringify(settings),
			},
	...
	getSettings: async (
		projectId: string,
	): Promise<EtapSettingsResponse | null> =>
		apiCall(
			`/integrations/etap/settings?project_id=${encodeURIComponent(projectId)}`,
		),
	...
	updateSettings: async (
		projectId: string,
		data: Partial<EtapConnectionSettings> & { enabled?: boolean },
	): Promise<EtapSettingsResponse> =>
		apiCall(
			`/integrations/etap/settings?project_id=${encodeURIComponent(projectId)}`,
			{
...
export const fullApi = {
	...
	getMe: async () => apiCall<{ data?: Record<string, unknown> }>("/auth/me"),
	verifyToken: async (token: string) =>
		apiCall<{ success: boolean }>("/auth/verify", {
			method: "POST",
	...
	qomnCalculate: async (
		...
		if (params === null || params === undefined || typeof params !== "object") {
			return {
				success: false,
				message:
					"qomnCalculate: params must be a non-null object. Use qomnApi.smokeSpacing/battery/voltageDrop directly for typed calls.",
			};
		}
		...
		const detectedLegacy = legacyAmbiguousKeys.filter(
		...
		if (params.ceiling_height_m !== undefined) {
			return apiCall<{
				...
				method: "POST",
				body: JSON.stringify({
					ceiling_height_m: Number(params.ceiling_height_m),
				}),
			});
		}
		if (
			params.standby_load_a !== undefined ||
			params.alarm_load_a !== undefined
		) {
			return apiCall<{
				success: boolean  ;
				data?: Record<string, unknown>;
	...
	getEnvironmentalContext: async (lat: number, lon: number) =>
		apiCall<{
			  data?: Record<
				string,
				unknown
			>;
		}>(`/environment/context?lat=${lat}&lon=${lon}`),
	getWeatherForecast: async (location: string) =>
		apiCall<{
			  data?: Record<
				...
				unknown
			>;
		}>(`/environment/weather?location=${encodeURIComponent(location)}`),
	getAirQualityData: async (lat: number, lon: number) =>
		apiCall<{
			  data?: Record<
				string,
				unknown
			>;
		}>(`/environment/air-quality?lat=${lat}&lon=${lon}`),
};
...
export const adminApi = {
	...
	clearCache: () =>
		apiCall<{ success: boolean; message: string }>("/cache/clear", {
			method: "POST",
		}),

	/** GET /feature-flags — Get all feature flag states */
	getFeatureFlags: () =>
		apiCall<{ success: boolean; data: Record<string, boolean> }>(
			"/feature-flags",
		),
	...
	rotateSecret: (data: {
		...
		apiCall<{ success: boolean; message: string }>(
			"/settings/secret-rotation/rotate",
			{
				method: "POST",
				body: JSON.stringify(data),
			},
	...
	rotateAdminToken: (data: {
		new_token: string;
		grace_period_seconds?: number;
	}) =>
		apiCall<{ success: boolean; message: string }>(
			"/settings/admin-token/rotate",
			{
				method: "POST",
				body: JSON.stringify(data),
			},
	...
	getDatabaseHealth: () =>
		apiCall<{
			...
			>;
		}>("/database-health", undefined, API_BASE.replace("/v1", "")),
...
export const qomnExtendedApi = {
	...
	runGoldenTests: () => apiCall("/qomn/golden-tests", { method: "POST" }),
...
export const marineExtendedApi = {
	...
	generateAlarmLogic: (data: { ship_id: string; zone_ids?: string[] }) =>
		apiCall("/marine/generate-alarm-logic", {
			method: "POST",
			body: JSON.stringify(data),
		}),
	...
	generateDivisions: (data: { ship_id: string; division_count?: number }) =>
		apiCall("/marine/generate-divisions", {
			method: "POST",
			body: JSON.stringify(data),
		}),
...
export const copilotExtendedApi = {
	...
	translateModel: (data: {
		...
		apiCall("/engineering-copilot/translate-model", {
			method: "POST",
			body: JSON.stringify(data),
		}),
	...
	validateModel: (data: {
		...
		apiCall("/engineering-copilot/validate-model", {
			method: "POST",
			body: JSON.stringify(data),
		}),
	...
	generateReports: (data: { project_id?: string; report_types?: string[] }) =>
		apiCall("/engineering-copilot/generate-reports", {
			method: "POST",
			body: JSON.stringify(data),
		}),
};
...
export const v2ExtendedApi = {
	...
	cacheBimElement: (data: {
		element_id: string;
		element_data: Record<string, unknown>;
		ttl_seconds?: number;
	}) =>
		apiCall(
			"/multi-db/bim/cache-element",
			{ method: "POST", body: JSON.stringify(data) },
			API_V2_BASE,
		),
	...
	getCachedBimElement: (elementId: string) =>
		apiCall(
			`/multi-db/bim/get-cached-element/${elementId}`,
			undefined,
			API_V2_BASE,
		),
	...
	createRelationships: (data: {
		...
		apiCall(
			"/multi-db/bim/create-relationships",
			{ method: "POST", body: JSON.stringify(data) },
			API_V2_BASE,
		),
	...
	getRelatedElements: (elementId: string) =>
		apiCall(
			`/multi-db/bim/related-elements/${elementId}`,
			undefined,
			API_V2_BASE,
		),
};
...
export const cadApi = {
	...
	disconnect: () => apiCall("/cad/disconnect", { method: "POST" }),

	/** GET /cad/status — Get CAD connection status */
	getStatus: () => apiCall("/cad/status", { method: "GET" }),
...
export const analyzeApi = {
	...
	room: (
		projectId: string,
		data: { room_name?: string; area_m2?: number; ceiling_height_m?: number },
	) =>
...
export const adminConfigApi = {
	...
	getFeatureFlags: () => apiCall("/feature-flags", { method: "GET" }),
	...
	getEnvConfig: () => apiCall("/env-config", { method: "GET" }),
...
export const connectionsV2Api = {
	...
	list: () => apiCall("/connections", { method: "GET" }),
	...
	update: (connectionId: string, data: Record<string, unknown>) =>
		apiCall(`/connections/${encodeURIComponent(connectionId)}`, {
			method: "PUT",
			body: JSON.stringify(data),
		}),
...
export const elementsApi = {
	...
	list: () => apiCall("/elements", { method: "GET" }),
	...
	update: (elementId: string, data: Record<string, unknown>) =>
		apiCall(`/elements/${encodeURIComponent(elementId)}`, {
			method: "PUT",
			body: JSON.stringify(data),
		}),
...
export const copilotApi = {
	...
	chat: (data: { request: string; context?: Record<string, unknown> }) =>
		apiCall("/engineering-copilot/chat", {
			method: "POST",
			body: JSON.stringify(data),
		}),
	...
	processRequest: (data: {
		...
		apiCall("/engineering-copilot/process-request", {
			method: "POST",
			body: JSON.stringify(data),
		}),
	...
	createEntity: (data: {
		...
		apiCall("/engineering-copilot/create-entity", {
			method: "POST",
			body: JSON.stringify(data),
		}),

	/** GET /engineering-copilot/health — Health check */
	getHealth: () => apiCall("/engineering-copilot/health", { method: "GET" }),
...
export const environmentRegionApi = {
	...
	getRegion: (params?: { lat?: number; lon?: number; address?: string }) => {
		...
		if (params?.address) query.set("address", params.address);
		const qs = query.toString();
		return apiCall(`/environment/region${qs ? `?${qs}` : ""}`, {
			method: "GET",
		});
	},
};
...
export const experimentalApi = {
	...
	processScanToBim: (data: { file_url?: string; project_id?: string }) =>
		apiCall("/experimental/scan-to-bim/process", {
			method: "POST",
			body: JSON.stringify(data),
		}),
	...
	specklePush: (data: {
		...
		apiCall("/experimental/speckle/push", {
			method: "POST",
			body: JSON.stringify(data),
		}),
	...
	speckleReceive: (data: { stream_id?: string; commit_id?: string }) =>
		apiCall("/experimental/speckle/receive", {
			method: "POST",
			body: JSON.stringify(data),
		}),
};
...
export const llmExtendedApi2 = {
	...
	getHealth: () => apiCall("/llm/health", { method: "GET" }),
};
...
export const rbacApi = {
	...
	getPermissions: () => apiCall("/admin/rbac/permissions", { method: "GET" }),
};
...
export const settingsApi = {
	...
	storeKey: (provider: string, data: { key: string; label?: string }) =>
		apiCall(`/settings/keys/${encodeURIComponent(provider)}`, {
			method: "POST",
			body: JSON.stringify(data),
		}),
	...
	listOpenaiKeys: () => apiCall("/settings/keys/openai", { method: "GET" }),

	/** GET /settings/keys/{provider}/{key_id} — Get a specific key */
	getKey: (provider: string, keyId: string) =>
		apiCall(
			`/settings/keys/${encodeURIComponent(provider)}/${encodeURIComponent(keyId)}`,
			{ method: "GET" },
		),
	...
	bulkDeleteKeys: (provider: string, data: { key_ids: string[] }) =>
		apiCall(`/settings/keys/${encodeURIComponent(provider)}/bulk-delete`, {
			method: "POST",
			body: JSON.stringify(data),
		}),
	...
	testKey: (provider: string, keyId: string) =>
		apiCall(
			`/settings/keys/${encodeURIComponent(provider)}/${encodeURIComponent(keyId)}/test`,
			{ method: "POST" },
		),

	/** GET /settings/keys/openai/{key_id} — Get OpenAI key (compat alias) */
	getOpenaiKey: (keyId: string) =>
		apiCall(`/settings/keys/openai/${encodeURIComponent(keyId)}`, {
			method: "GET",
		}),
	...
	testOpenaiKey: (keyId: string) =>
		apiCall(`/settings/keys/openai/${encodeURIComponent(keyId)}/test`, {
			method: "POST",
		}),
};
...
export const digitalTwinUploadApi = {
	...
	uploadAndConvert: (
		file: File,
		options?: { target_format?: string; mapping_profile?: string },
	) => {
		const formData = new FormData();
		formData.append("file", file);
		if (options?.target_format)
			formData.append("target_format", options.target_format);
		if (options?.mapping_profile)
			formData.append("mapping_profile", options.mapping_profile);
...
export const apsApi = {
	...
	getStatus: (workItemId: string) =>
		apiCall(
			`/aps/status/${encodeURIComponent(workItemId)}`,
			{ method: "GET" },
			API_V2_BASE,
		),
};
...
export const revitIntegrationApi = {
	...
	uploadModel: (projectId: string, file: File) => {
		...
		formData.append("file", file);
		return apiCall(
			`/revit-integration/upload?project_id=${encodeURIComponent(projectId)}`,
			{
				method: "POST",
				body: formData,
				headers: {},
	...
	getModel: (modelId: string) => apiCall(`/revit-integration/model/${modelId}`),
	...
	getSyncStatus: (projectId: string) =>
		apiCall(
			`/revit-integration/status?project_id=${encodeURIComponent(projectId)}`,
		),
};
...
export const reportsApi = {
	...
	list: (
		...
		const qs = query.toString();
		return apiCall(
			`/projects/${encodeURIComponent(projectId)}/reports${qs ? `?${qs}` : ""}`,
		);
	},
	...
	generate: (
		projectId: string,
		data: { report_type: string; config?: Record<string, unknown> },
	) =>
	...
	exportReport: (projectId: string, reportId: string, format: string = "pdf") =>
		apiCall(
			`/projects/${encodeURIComponent(projectId)}/reports/${encodeURIComponent(reportId)}/export?format=${encodeURIComponent(format)}`,
		),

	/** POST /projects/{project_id}/reports/ahj-submittal — Generate AHJ submittal document */
	generateAhjSubmittal: (
		projectId: string,
		data: { designer?: string; jurisdiction?: string; nfpa_edition?: string },
	) =>
		apiCall(
			`/projects/${encodeURIComponent(projectId)}/reports/ahj-submittal`,
			{
				method: "POST",
				body: JSON.stringify(data),
			},

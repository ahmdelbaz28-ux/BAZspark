...
# These functions are required by fire_expert_system.py V10
# ============================================================================
...
def calculate_max_spacing(
    ...
    """
    Return the allowable centre-to-centre spacing between detectors.

    NFPA 72 §17.6.3 with height and beam corrections.
    Uses get_smoke_detector_coverage_max() which already incorporates
    height correction factors.

    Args:
        ceiling:       Validated ceiling specification.
        detector_type: Type of detector being evaluated.

    Returns:
        Maximum allowable spacing in metres.
    """
    # Get spacing from existing function (already has height correction)
    spacing = get_smoke_detector_coverage_max(ceiling.height_m)
    
    # Apply beam pocket correction if needed
    if ceiling.beam_depth_m > 0:
        beam_factor = beam_pocket_correction_factor(
            ceiling.beam_depth_m, ceiling.height_m
        )
        spacing *= beam_factor

    # Apply sloped ceiling correction (use high point)
    if ceiling.ceiling_type == CeilingType.SLOPED:
        spacing = get_smoke_detector_coverage_max(ceiling.height_m)
...
def calculate_coverage_radius(
    ...
    """
    Return the effective coverage radius for a single detector.
...
def calculate_max_wall_distance(
    ceiling: "CeilingSpec",
    detector_type: "DetectorType",
) -> float:
    """
    Return the maximum permitted distance from detector to wall.

    NFPA 72 §17.6.3.1.1: max_wall_distance = radius
    ...
    Returns:
        Maximum wall distance in metres.
    """
    return calculate_coverage_radius(ceiling, detector_type)


# Helper function for beam pocket correction (missing in nfpa72_models)
def beam_pocket_correction_factor(beam_depth_m: float, ceiling_height_m: float) -> float:
    """
    Return correction factor for beam pocket areas per NFPA 72 §17.6.3.6.

    When beams extend down more than 100mm (4in) from ceiling, spacing
    may need reduction. Simplified formula.

    Args:
        beam_depth_m:      Depth of beam below ceiling in metres.
        ceiling_height_m:   Height of ceiling in metres.

    Returns:
        Correction factor (typically 0.7-1.0).
    """
    if beam_depth_m < 0.1:  # < 100mm - no correction
        return 1.0
    if beam_depth_m > 0.4:  # > 400mm - maximum correction
        return 0.7
    # Linear interpolation
    return 1.0 - (beam_depth_m - 0.1) * 0.5

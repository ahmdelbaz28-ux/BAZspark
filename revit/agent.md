...
## V31 Rules Engine Integration (2026-05-29) — Declarative NFPA 72 Rules + Truth Maintenance + Type-Safe API Contract

### Context
After reading jruizgit/rules (durable_rules) and trpc/trpc repositories as an expert, analyzed how their concepts can strengthen FireAI. Applied the concept of Rete algorithm forward-chaining rules (from durable_rules) but rebuilt in pure Python for memory safety, thread safety, audit trail, and truth maintenance — all critical for a life-critical fire alarm system. Applied the concept of end-to-end type safety (from tRPC) but via OpenAPI code generation since tRPC is TypeScript-only and incompatible with our Python backend.

### New Modules (8 files, 3459 lines)

| File | Purpose |
|------|---------|
| `fireai/core/rules_engine/__init__.py` | Package initialization |
| `fireai/core/rules_engine/engine.py` | Core forward-chaining rules engine with Rete-inspired alpha/beta network, priority-based conflict resolution, full audit trail, thread-safe sessions |
| `fireai/core/rules_engine/truth_maintenance.py` | TMS — retracts derived conclusions when base facts change (prevents stale conclusions) |
| `fireai/core/rules_engine/nfpa72_rules.py` | 10 declarative NFPA 72 rules with structured metadata |
| `fireai/core/rules_engine/api_contract.py` | Type-safe API contract system (ContractValidator + OpenAPI generation) |
| `fireai/core/rules_engine/compliance_bridge.py` | High-level integration bridge (NFPA72ComplianceChecker API) |
| `fireai/core/rules_engine/test_rules_engine.py` | 50 engine tests |
| `fireai/core/rules_engine/test_compliance_bridge.py` | 17 bridge tests |

### NFPA 72 Declarative Rules

| Rule ID | Name | NFPA Reference | Priority |
|---------|------|---------------|----------|
| NFPA72-001 | Ceiling Height → Spacing | §17.6.3.1, Table 17.6.3.1 | COMPLIANCE_CHECK |
| NFPA72-002 | Coverage Radius = 0.7 × S | §17.7.4.2.3.1 | COMPLIANCE_CHECK |
| NFPA72-003 | Ceiling Height Exceeds Table Range | §17.6.3.1 Note | CRITICAL_SAFETY |
| NFPA72-004 | Dead Air Space | §17.6.3.1.1 | SAFETY_VIOLATION |
| NFPA72-005 | Wall Distance Exceeded | §17.6.3.1 | SAFETY_VIOLATION |
| NFPA72-006 | Duct Detector Required | §17.7.5.1 | SAFETY_VIOLATION |
| NFPA72-007 | Elevator Recall | §21.3.3 | SAFETY_VIOLATION |
| NFPA72-008 | Corridor Spacing | §17.7.3.1 | COMPLIANCE_CHECK |
| NFPA72-009 | Minimum Detector Count | §17.6.3.1, §17.7.4.2.3.1 | COMPLIANCE_CHECK |
| NFPA72-010 | Detector Spacing Violation (join) | §17.6.3.1 | CRITICAL_SAFETY |

### Key Features

1. **Declarative Rules**: NFPA 72 rules defined separately from code — auditable by FPEs without reading Python
2. **Full Audit Trail**: Every rule evaluation logged (fired or not) — complete decision traceability
3. **Truth Maintenance**: Derived facts retract when base facts change — prevents stale conclusions
4. **Priority-Based Conflict Resolution**: CRITICAL_SAFETY rules fire before ADVISORY rules — deterministic
5. **Thread-Safe Sessions**: Each analysis session isolated via session_id
6. **Derivation Explanation**: `engine.explain(fact_id)` traces why any fact exists
7. **Type-Safe API Contracts**: ContractValidator + OpenAPI generation for TypeScript frontend
8. **Fact Immutability**: Facts are frozen dataclasses — no accidental mutation

### Bug Found and Fixed

**Rule Double-Firing in Evaluation Loop**
- **Discovery**: Rule fired twice in the same evaluation cycle
- **Root Cause**: `fired_rule_ids` set was scoped to a single pass within the while loop, allowing the same rule+fact combination to fire again in subsequent passes
- **Fix**: Changed to `_fired_combinations` set (rule_id, frozenset of matched fact_ids) that persists across all passes within an evaluation cycle
- **Reference**: agent.md Rule 17 (No Half-Solutions) — the superficial fix would have been to use `fired_rule_ids` per pass; the root cause fix tracks exact rule+fact combinations

### Design Decisions

1. **Custom Python Rules Engine vs durable_rules**: durable_rules has C memory safety risks, no thread safety, no audit trail, no TMS. Our pure-Python engine provides all safety features at the cost of raw performance (acceptable for safety-critical systems where correctness > performance per agent.md Priority 4).

2. **OpenAPI Code Generation vs tRPC**: tRPC is TypeScript-only and incompatible with our Python FastAPI backend. OpenAPI spec generation + openapi-typescript achieves the same end-to-end type safety for our Python+TypeScript stack.

3. **Fact Immutability**: Facts are frozen dataclasses. A changed fact must be retracted and re-asserted, which triggers truth maintenance cascade. This prevents stale derived conclusions.

### Test Results
- **67/67 passing** (50 engine tests + 17 bridge tests)
- Categories: Fact basics, Engine basics, NFPA 72 rules, Truth maintenance, API contracts, Rule set metadata, Edge cases, Compliance checker, Data conversion

### Self-Criticism Notes (V31)

1. **Only 10 NFPA 72 rules defined** — NFPA 72 has hundreds of rules. The current 10 cover the most critical sections. More rules must be added incrementally with proper NFPA section references.

2. **No integration with existing AuditStore** — The rules engine has its own audit log but it's not yet written to the SQLite AuditStore. This must be connected for production use.

3. **No performance benchmarking** — The engine hasn't been stress-tested with large fact sets (1000+ rooms). Performance may need optimization for building-level analysis.

4. **api_contract.py is not integrated with FastAPI routes yet** — The ContractValidator exists but isn't wired into the actual API endpoints. Must be connected to backend/app.py.

5. **Missing join rule test** — RULE_DETECTOR_SPACING_VIOLATION (NFPA72-010) is a join rule but doesn't have a dedicated test in the test suite. Must add.

### Commit Information
- **Commit:** `6caccc6`
- **Link:** https://github.com/ahmdelbaz28-ux/revit/commit/6caccc6

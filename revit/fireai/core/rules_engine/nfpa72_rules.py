...
RULE_MINIMUM_DETECTOR_COUNT = Rule(
    ...
    action=lambda facts, engine: [
        RuleResult(
            ...
            message=(
                f"Room '{facts[0].properties.get('room_id', '?')}': "
                f"R={facts[0].properties['R_m']:.2f}m, "
                f"minimum detectors calculated"
            ),
            asserted_facts=[
                Fact(
                    ...
                    properties={
                        "room_id": facts[0].properties.get("room_id", ""),
                        "detector_type": facts[0].properties.get("detector_type", ""),
                        "coverage_radius_m": facts[0].properties["R_m"],
                        "min_detectors": _min_detectors_for_room(
                            # We need room area — derive from room facts
                            # For now, store R and let downstream code compute
                            1.0,  # placeholder — real area from room fact
                            facts[0].properties["R_m"],
                        ),
                        "needs_room_area": True,
                    },
                    source="derived",
                    nfpa_reference="NFPA 72 §17.7.4.2.3.1",
                )

...
class RevitElementAdapter(IRevitAdapter):
    ...
    def convert_to_dto(self, revit_element: Any) -> RevitElementDTO:
        ...
        try:
            ...
            family = (
                getattr(getattr(revirt_element, "Family", None), "Name", "")
                if hasattr(revit_element, "Family")
                else ""
            )
            element_type = (
                getattr(getattr(revit_element, "Symbol", None), "FamilyName", "")
                if hasattr(revit_element, "Symbol")
                else ""
            )

...
def api_call(
    backend: str, admin_key: str, master_token: str, method: str, path: str, body: dict = None
) -> dict:
    ...
    )  # NOSONAR — S8703: backend URL is validated via argparse defaults
    try:
        ...
        if e.code == 401:
            ...
            print(
                "   → Forbidden: Master admin token required (X-Master-Admin-Token header).",
                file=sys.stderr,
            )
            print("   → Or: The API key does not have admin permissions.", file=sys.stderr)
        elif e.code == 429:
            print(
                "   → Rate limited (10 ops/minute). Wait 60 seconds and try again.", file=sys.stderr
            )
        sys.exit(1)
...
def generate_key(
    backend: str, admin_key: str, master_token: str, role: str, description: str
) -> None:

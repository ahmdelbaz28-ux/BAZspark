...
class TestEtapCrypto:
    ...
    def test_decrypt_password_returns_original(self):
        ...
        decrypted = __import__(
            "backend.integrations.etap_crypto", fromlist=["decrypt_password"]
...
class TestEtapDatabase:
    ...
    def test_etap_integrations_table_exists(self):
        ...
        try:
            with db._transaction() as cur:
                cur.execute(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='etap_integrations'"
                )
                row = cur.fetchone()
    ...
    def test_etap_sync_logs_table_exists(self):
        ...
        try:
            with db._transaction() as cur:
                cur.execute(
                    "SELECT name FROM sqlite_master WHERE type='table' AND name='etap_sync_logs'"
                )
                row = cur.fetchone()
                assert row is not None
        finally:
    ...
    def test_etap_integrations_index_exists(self):
        ...
        try:
            with db._transaction() as cur:
                cur.execute(
                    "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_etap_integrations_project'"
                )
                row = cur.fetchone()
                assert row is not None
        finally:

"""
tests/test_smoke_spacing_audit_v120.py - V127 Height-Adjusted Spacing + V120 Audit Tests
=========================================================================================
V127 FIX: Smoke detector spacing is height-adjusted per
SMOKE_HEIGHT_SPACING_TABLE (10 rows). V120 introduced the audit_notice
key. In V127 the audit_notice is ALWAYS present and contains BOTH
"V120 AUDIT WARNING" and "SMOKE_SPACING_AUDIT_FINDING_1.md".

Per NFPA 72-2022 §17.7.3.2.3 (verbatim):
  "Spot-type smoke detectors shall be spaced not more than
   30 ft (9.1 m) apart on smooth ceilings."

The V127 table is a CONSERVATIVE fail-safe fallback pending licensed
FPE sign-off. A V120 AUDIT WARNING is always emitted.

These tests verify:
  1. V127 table spacing at each table row
  2. The V120 audit_notice is always present and references finding doc
  3. The runtime WARNING log fires with "V120 AUDIT WARNING"
  4. Pre-existing physics guards are preserved
"""

from __future__ import annotations

import logging
import sys
from pathlib import Path

_PROJECT_ROOT = Path(__file__).parent.parent.resolve()
if str(_PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(_PROJECT_ROOT))

import pytest

from fireai.core.qomn_kernel import compute_smoke_detector_spacing


# ===============================================================================
# V127 - Height-Adjusted Spacing at each table row
# ===============================================================================


class TestV127HeightAdjustedSpacing:
    """V127: Spacing is height-adjusted per SMOKE_HEIGHT_SPACING_TABLE."""

    @pytest.mark.parametrize("height, expected", [
        (3.0,    9.10),   # 10 ft - row 0
        (3.048,  8.70),   # exactly 10 ft -> falls in row 1 (h<=3.7)
        (3.658,  8.70),   # 12 ft -> row 1
        (4.572,  8.20),   # 15 ft -> row 2 (h<=4.6)
        (6.096,  7.30),   # 20 ft -> row 4 (h<=6.1)
        (9.144,  6.40),   # 30 ft exact -> row 6 (h<=9.144)
        (18.288, 5.20),   # 60 ft (hard limit) -> row 9
    ])
    def test_h_value(self, height, expected):
        """Verify V127 spacing at each test height."""
        r = compute_smoke_detector_spacing(height)
        assert r["listed_spacing_m"] == pytest.approx(expected, abs=1e-3), (
            f"At h={height}m, expected S={expected}m, "
            f"got S={r['listed_spacing_m']}m"
        )

    def test_coverage_radius(self):
        """Coverage radius = 0.7 x spacing per NFPA 72 §17.7.4.2.3.1."""
        for h in (3.0, 4.0, 5.0, 6.0, 9.0, 15.0):
            r = compute_smoke_detector_spacing(h)
            assert r["coverage_radius_m"] == pytest.approx(
                0.7 * r["listed_spacing_m"], rel=1e-4
            )

    def test_nfpa_section_in_result(self):
        """The nfpa_section key must cite §17.7.3.2.3."""
        r = compute_smoke_detector_spacing(3.0)
        assert "NFPA 72" in r["nfpa_section"]
        assert "17.7.3.2.3" in r["nfpa_section"]

    def test_computation_hash_present(self):
        """Audit hash still computed."""
        r = compute_smoke_detector_spacing(3.0)
        assert len(r["computation_hash"]) > 0


# ===============================================================================
# V120 - Audit Notice (always emitted in V127)
# ===============================================================================


class TestV120AuditNotice:
    """V120: The audit_notice is ALWAYS emitted in V127 and references
    SMOKE_SPACING_AUDIT_FINDING_1.md."""

    def test_audit_notice_always_present(self):
        """audit_notice is always present, even at low ceilings."""
        for h in (1.0, 3.0, 4.0, 5.0, 6.0, 6.096, 7.0, 9.0, 12.0, 15.0, 18.0):
            r = compute_smoke_detector_spacing(h)
            assert "audit_notice" in r, f"audit_notice missing at h={h}"
            assert isinstance(r["audit_notice"], str)
            assert len(r["audit_notice"]) > 0

    def test_audit_notice_cites_v120(self):
        """Notice must contain 'V120 AUDIT WARNING'."""
        r = compute_smoke_detector_spacing(10.0)
        assert "V120 AUDIT WARNING" in r["audit_notice"]

    def test_audit_notice_references_finding_doc(self):
        """Notice must reference SMOKE_SPACING_AUDIT_FINDING_1.md."""
        r = compute_smoke_detector_spacing(10.0)
        assert "SMOKE_SPACING_AUDIT_FINDING_1.md" in r["audit_notice"]

    def test_audit_notice_cites_stratification(self):
        """The notice must cite NFPA 72 §17.7.1.11 (stratification)."""
        r = compute_smoke_detector_spacing(10.0)
        assert "17.7.1.11" in r["audit_notice"]

    def test_audit_notice_offers_alternatives(self):
        """The notice must direct the operator toward beam, aspirating,
        OR performance-based design."""
        r = compute_smoke_detector_spacing(10.0)
        notice = r["audit_notice"].lower()
        assert "beam" in notice
        assert "aspirating" in notice or "air-sampling" in notice


# ===============================================================================
# V120 - Runtime WARNING log
# ===============================================================================


class TestV120WarningLog:
    """V120 logs a WARNING at the kernel's logger. In V127 this fires
    every call (audit warning is unconditional)."""

    def test_warning_emitted(self, caplog):
        with caplog.at_level(logging.WARNING, logger="fireai.core.qomn_kernel"):
            compute_smoke_detector_spacing(10.0)
        warning_messages = [
            r.message for r in caplog.records
            if r.levelno == logging.WARNING
        ]
        assert any("V120 AUDIT WARNING" in m for m in warning_messages), (
            f"Expected V120 AUDIT WARNING log; got: {warning_messages}"
        )

    def test_warning_emitted_at_low_ceiling_too(self, caplog):
        """V127 emits the V120 audit warning at ALL heights."""
        with caplog.at_level(logging.WARNING, logger="fireai.core.qomn_kernel"):
            compute_smoke_detector_spacing(3.0)
        warning_messages = [
            r.message for r in caplog.records
            if r.levelno == logging.WARNING and "V120" in r.message
        ]
        assert warning_messages, (
            f"Expected V120 AUDIT WARNING log at h=3m; got: {warning_messages}"
        )

    def test_logging_failure_does_not_break_computation(self, monkeypatch):
        """If logging fails for any reason, the engineering computation
        MUST still return correctly."""
        import logging as _real_logging
        original_get_logger = _real_logging.getLogger

        def broken_get_logger(name):
            raise RuntimeError("Simulated logging failure")
        monkeypatch.setattr(_real_logging, "getLogger", broken_get_logger)

        try:
            r = compute_smoke_detector_spacing(10.0)
            assert r["listed_spacing_m"] > 0
            assert "audit_notice" in r
        finally:
            monkeypatch.setattr(_real_logging, "getLogger", original_get_logger)


# ===============================================================================
# V120 Phase A - Existing kernel exceptions preserved
# ===============================================================================


class TestV120ExistingGuardsPreserved:
    """V120 must not weaken any pre-existing physics guards."""

    def test_nan_still_rejected(self):
        from fireai.core.qomn_kernel import PhysicsGuardError
        with pytest.raises(PhysicsGuardError):
            compute_smoke_detector_spacing(float("nan"))

    def test_inf_still_rejected(self):
        from fireai.core.qomn_kernel import PhysicsGuardError
        with pytest.raises(PhysicsGuardError):
            compute_smoke_detector_spacing(float("inf"))

    def test_negative_still_rejected(self):
        from fireai.core.qomn_kernel import PhysicsGuardError
        with pytest.raises(PhysicsGuardError):
            compute_smoke_detector_spacing(-1.0)

    def test_zero_still_rejected(self):
        from fireai.core.qomn_kernel import PhysicsGuardError
        with pytest.raises(PhysicsGuardError):
            compute_smoke_detector_spacing(0.0)

    def test_above_18288m_still_rejected(self):
        """NFPA global scope ceiling unchanged at 18.288 m."""
        from fireai.core.qomn_kernel import PhysicsGuardError
        with pytest.raises(PhysicsGuardError):
            compute_smoke_detector_spacing(18.5)
